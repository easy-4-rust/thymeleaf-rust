...
fn engine_with_resolver(resolver: Arc<dyn ITemplateResolver>) -> TemplateEngine {
    ...
    engine.set_template_resolver(resolver).expect("set resolver");
    engine
}
...
fn parsing_engine(character_encoding: &str) -> TemplateEngine {
    let mut resolver = thymeleaf::templateresolver::ClassLoaderTemplateResolver::with_search_roots(
        vec![fixtures_dir()],
    );
    resolver.set_template_mode_name(Some("HTML5")).expect("HTML5 template mode");
...
const BLOCK_SELECTOR_FIXTURES: &[(&str, &str)] = &[
    ("htmlblockselector/test001.html", "htmlblockselector/result001.html"),
    ("htmlblockselector/test002.html", "htmlblockselector/result002.html"),
    ("htmlblockselector/test003.html", "htmlblockselector/result003.html"),
];

const BLOCK_SELECTOR_ASSET_SHA256: &[(&str, &str)] = &[
    ("htmlblockselector/test001.html", "996cd829b18fef41a28ee964d915bb551d811880117de572b4d0c7780de20114"),
    ("htmlblockselector/test002.html", "3fb81e95e78db331a6a74096a902288cbb23b2733174b39d7f0f355670510275"),
    ("htmlblockselector/test003.html", "49726d2db91d5e97f76c4a9f30e2c461f9420c4a359a92b042e98d1ee1484419"),
    ("htmlblockselector/result001.html", "4b8083ea96768a7c552e862087030e23ff40852d0ec398e197443951df536355"),
    ("htmlblockselector/result002.html", "eb3cc1f0658e77d19d7d5c8fd7e7c625aba729a6b4ae4713b76cdee960065ebc"),
    ("htmlblockselector/result003.html", "0f7239ee69ab8b976cb8b5824555f46644713212e1e997504a1fd8d2ec840b26"),
];
...
fn html_block_selector_matches_java() {
    ...
    for (test_file, result_file) in BLOCK_SELECTOR_FIXTURES {
        ...
        let selectors: Vec<JavaString> =
            block_selector.split(',').map(js).collect();

        let resource: Arc<dyn ITemplateResource> = Arc::new(
            StringTemplateResource::new(Some(&test_contents)).expect("string resource"),
        );
        ...
        parser
            ...
            .unwrap_or_else(|error| {
                panic!("{test_file} 块选择器解析失败: {error}")
            });
...
fn assert_resolver_html(html: bool, prefix: Option<&str>, expected: &str) {
    let resolver = TemplateFragmentMarkupReferenceResolver::for_prefix(html, prefix.map(js).as_ref());
...
fn text_mode_parsers_roundtrip_identity() {
    ...
    let javascript: Arc<dyn thymeleaf::templateparser::ITemplateParser> =
        Arc::new(thymeleaf::text::JavaScriptTemplateParser::new(2, 4096, true));
