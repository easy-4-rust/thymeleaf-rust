...
fn web_exchange_contract_matches_java() {
    ...
    assert_eq!(
        exchange.get_request().get_method().unwrap().to_string_lossy(),
        "GET"
    );
    assert!(exchange.has_session(), "corpus exchange always has a session");
    ...
    exchange.set_attribute_value(Some(js("attr1")), Some(Arc::new(TemplateValue::string(js("v")))));
    assert!(exchange.contains_attribute(Some(&js("attr1"))));
    assert_eq!(
        exchange
...
fn web_session_and_application_contract_matches_java() {
    ...
    session.set_attribute_value(Some(js("s1")), Some(Arc::new(TemplateValue::string(js("sv")))));
    assert!(session.contains_attribute(Some(&js("s1"))));
    assert_eq!(
        session
    ...
    application.set_attribute_value(Some(js("a1")), Some(Arc::new(TemplateValue::string(js("av")))));
    assert!(application.contains_attribute(Some(&js("a1"))));
    assert_eq!(
        application
