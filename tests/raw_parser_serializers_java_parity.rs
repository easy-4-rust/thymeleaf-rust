...
impl IRawHandler for RecordingRawHandler {
    fn handle_document_start(
        ...
        self.log
            .push(format!("start(nanos={start_time_nanos},line={line},col={col})"));
        Ok(())
    }
...
fn raw_parser_document_events_match_java() {
    ...
    assert!(handler.log[0].starts_with("start(nanos="), "start event shape");
    ...
    assert!(handler.log[2].contains(",line=1,col=21)"), "end col = 内容长度");
...
fn raw_parse_exception_contract_matches_java() {
    ...
    let exception = RawParseException::with_message_and_cause(
        Some(js("at 3:5")),
        Some(RawParseCause::from_raw_parse(RawParseException::with_message(
            Some(js("inner")),
        ))),
    );
...
fn standard_serializers_execution_attributes_match_java() {
    ...
    let css_serializer = StandardSerializers::get_css_serializer(configuration.as_ref())
        .expect("css serializer");
...
fn standard_processor_utils_attribute_helpers_match_java() {
    ...
    impl thymeleaf::element::IElementTagStructureHandler for RecordingStructureHandler {
        fn reset(&mut self) {}
        fn set_local_variable(&mut self, _name: JavaString, _value: Option<Arc<thymeleaf::expression::TemplateValue>>) {}
        ...
        fn set_attribute(
            ...
            self.log.push(format!(
                ...
                attribute_value.map(|v| v.to_string_lossy()).unwrap_or_default()
            ));
        }
        ...
        fn remove_attribute_name(&mut self, _attribute_name: thymeleaf::engine::AttributeNameValue) {}
        fn set_selection_target(&mut self, _selection_target: Option<Arc<thymeleaf::expression::TemplateValue>>) {}
        ...
        fn set_body_sequence(&mut self, _text: Arc<dyn thymeleaf::util::JavaCharSequence>, _processable: bool) {}
        fn set_body_model(&mut self, _model: Arc<dyn thymeleaf::model::IModel>, _processable: bool) {}
        fn insert_before(&mut self, _model: Arc<dyn thymeleaf::model::IModel>) {}
        fn insert_immediately_after(&mut self, _model: Arc<dyn thymeleaf::model::IModel>, _processable: bool) {}
        fn replace_with_text(&mut self, _text: JavaString, _processable: bool) {}
        fn replace_with_model(&mut self, _model: Arc<dyn thymeleaf::model::IModel>, _processable: bool) {}
    ...
    let old_name = thymeleaf::engine::AttributeNames::for_name(
