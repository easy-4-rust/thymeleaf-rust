...
use thymeleaf::model::{IProcessableElementTag, IModel};
...
fn element_name(name: &str) -> thymeleaf::engine::ElementNameValue {
    ElementNames::for_name(Some(TemplateMode::HTML), Some(&js(name)))
        .expect("element name")
}
...
fn matching_attribute_name_rules_match_java() {
    ...
    let matching =
        MatchingAttributeName::for_attribute_name(Some(TemplateMode::HTML), Some(attribute_name("th:value")))
...
fn matching_element_name_rules_match_java() {
    ...
    let prefixed = MatchingElementName::for_all_elements_with_prefix(
        Some(TemplateMode::HTML),
        Some(js("th")),
    )
    .expect("prefixed rule");
...
fn abstract_attribute_tag_processor_contract_matches_java() {
    ...
    assert_eq!(processor.java_class_name(), "com.example.MyTagAttrProcessor");
    assert_eq!(
        processor
...
fn abstract_element_tag_processor_contract_matches_java() {
    ...
    assert_eq!(processor.java_class_name(), "com.example.MyElementProcessor");
    assert_eq!(
        processor
