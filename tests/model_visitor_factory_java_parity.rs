...
use thymeleaf::engine::{
    AttributeNames, TemplateData, TemplateEnd, TemplateStart,
};
use thymeleaf::model::{
...
use thymeleaf::util::{FastStringWriter, JavaCharSequence, JavaString};
...
fn write_events(events: &[&dyn ITemplateEvent]) -> String {
    ...
    for event in events {
        event
            .write(&mut writer)
            .expect("event write must not fail");
    }
...
impl IModelVisitor for RecordingVisitor {
    fn visit_template_start(&mut self, _template_start: &dyn ITemplateStart) {
        self.log.borrow_mut().push("visit_template_start".to_owned());
    }
    ...
    fn visit_xml_declaration(&mut self, _xml_declaration: &dyn IXMLDeclaration) {
        self.log.borrow_mut().push("visit_xml_declaration".to_owned());
    }
    ...
    fn visit_standalone_element_tag(&mut self, _standalone_element_tag: &dyn IStandaloneElementTag) {
        self.log.borrow_mut().push("visit_standalone_element_tag".to_owned());
    }
    fn visit_open_element_tag(&mut self, _open_element_tag: &dyn IOpenElementTag) {
        self.log.borrow_mut().push("visit_open_element_tag".to_owned());
    }
    fn visit_close_element_tag(&mut self, _close_element_tag: &dyn ICloseElementTag) {
        self.log.borrow_mut().push("visit_close_element_tag".to_owned());
    }
    fn visit_processing_instruction(&mut self, _processing_instruction: &dyn IProcessingInstruction) {
        self.log.borrow_mut().push("visit_processing_instruction".to_owned());
    }
}
...
fn factory_markup_events_serialization_matches_java() {
    with_factory(TemplateMode::HTML, |factory| {
        ...
        assert_eq!(write_events(&[as_event(comment.as_ref())]), "<!--a comment-->");
        ...
        assert_eq!(declaration.get_encoding().unwrap().to_string_lossy(), "UTF-8");
        assert_eq!(declaration.get_standalone().unwrap().to_string_lossy(), "yes");
        // 全部字段为空 -> "<?xml?>"
        let bare = factory
...
fn factory_element_tags_contract_matches_java() {
    with_factory(TemplateMode::HTML, |factory| {
        ...
        assert_eq!(write_events(&[as_event(open.as_ref())]), "<div class=\"a\">");
        ...
        assert_eq!(synthetic.get_element_complete_name().to_string_lossy(), "img");
...
fn factory_attribute_operations_matches_java() {
    with_factory(TemplateMode::HTML, |factory| {
        ...
        assert_eq!(write_events(&[as_event(with_class.as_ref())]), "<div class=\"b\">");
        ...
        assert!(removed.get_attribute_value(&js("class")).expect("value").is_none());
...
fn model_accept_visitor_dispatch_order_matches_java() {
    with_factory(TemplateMode::HTML, |factory| {
        ...
        model
            ...
            .expect("add doctype");
        model
            .add(Some(factory.create_cdata_section(js("data")).expect("cdata")))
            .expect("add cdata");
        model
            .add(Some(factory.create_comment(js("c")).expect("comment")))
            .expect("add comment");
        model.add(Some(factory.create_text(js("t")))).expect("add text");
        model
            .add(Some(
                factory
                    .create_standalone_element_tag(
...
fn abstract_model_visitor_noop_defaults_match_java() {
    with_factory(TemplateMode::HTML, |factory| {
        let mut model = factory.create_model();
        model.add(Some(factory.create_text(js("a")))).expect("add text");
        model
            .add(Some(factory.create_comment(js("b")).expect("comment")))
            .expect("add comment");
        model.add(Some(factory.create_text(js("c")))).expect("add text");
        ...
        let visitor = OnlyTextVisitor { log: Rc::clone(&log) };
        {
            let mut visitor = visitor;
            model.accept(&mut visitor);
        }
...
fn factory_parse_roundtrip_matches_java() {
    with_factory(TemplateMode::HTML, |factory| {
        ...
        let full = factory
            ...
            .expect("parse markup");
        assert_eq!(full.size(), 6, "start + doctype + open + text + close + end");
