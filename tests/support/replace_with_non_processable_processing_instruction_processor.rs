...
/// 用固定 `<replaced th:text="one"/>` 模型替换 ProcessingInstruction 事件，替换模型不再处理。
///
/// 对应 Java:
/// `org.thymeleaf.templateengine.processors.dialects.replacewithnonprocessable.ReplaceWithNonProcessableProcessingInstructionProcessor`。
pub struct ReplaceWithNonProcessableProcessingInstructionProcessor {
    processor: AbstractProcessingInstructionProcessor<ProcessCallback>,
}

impl ReplaceWithNonProcessableProcessingInstructionProcessor {
...
impl Default for ReplaceWithNonProcessableProcessingInstructionProcessor {
    fn default() -> Self {
        Self::new()
    }
}

impl IProcessor for ReplaceWithNonProcessableProcessingInstructionProcessor {
...
impl IProcessingInstructionProcessor for ReplaceWithNonProcessableProcessingInstructionProcessor {
    fn process(
        &self,
