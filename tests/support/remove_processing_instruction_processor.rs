...
use thymeleaf::processinginstruction::{AbstractProcessingInstructionProcessor, IProcessingInstructionProcessor, IProcessingInstructionStructureHandler};

type ProcessResult = Result<(), Box<dyn TemplateEngineException>>;
type ProcessCallback =
    fn(&dyn ITemplateContext, &dyn IProcessingInstruction, &mut dyn IProcessingInstructionStructureHandler) -> ProcessResult;
...
impl IProcessor for RemoveProcessingInstructionProcessor {
    fn as_processing_instruction_processor(&self) -> Option<&dyn IProcessingInstructionProcessor> { Some(self) }
