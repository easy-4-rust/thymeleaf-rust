...
fn each_parse_matches_java() {
    ...
    let each = EachUtils::parse_each(context.as_ref(), Some(&js("v : ${list}")))
        .expect("parse each");
    assert_eq!(
        each.get_iter_var().get_string_representation().expect("iter var").to_string_lossy(),
        "v"
    );
    assert!(!each.has_status_var(), "no status var");
    assert_eq!(
        each.get_iterable().get_string_representation().expect("iterable").to_string_lossy(),
        "${list}"
    );
    assert_eq!(
        each.get_string_representation().expect("string form").to_string_lossy(),
        "v : ${list}"
    );
    ...
    assert_eq!(
        each.get_status_var().expect("status var").get_string_representation().expect("status text").to_string_lossy(),
        "st"
    );
    assert_eq!(
        each.get_string_representation().expect("string form").to_string_lossy(),
    ...
    assert_eq!(
        again.get_iter_var().get_string_representation().expect("iter var").to_string_lossy(),
        "v"
    );
}
...
fn assignation_sequence_parse_matches_java() {
    ...
    assert_eq!(
        first.get_left().get_string_representation().expect("left").to_string_lossy(),
        "a"
    );
    assert_eq!(
        first.get_right().expect("right").get_string_representation().expect("right text").to_string_lossy(),
        "${x}"
    );
    assert_eq!(
        sequence.get_string_representation().expect("string form").to_string_lossy(),
        "a=${x},b=${y}"
    );

    // 无值参数：allowParametersWithoutValue=false 拒绝
    let error = AssignationUtils::parse_assignation_sequence(
        context.as_ref(),
        Some(&js("a")),
        false,
    )
    ...
    assert!(error.to_string().contains("Could not parse as assignation sequence"));
...
fn expression_sequence_parse_matches_java() {
    ...
    assert!(error.to_string().contains("Could not parse as expression sequence"));
}
...
fn selection_variable_expression_matches_java() {
    ...
    let selection = SelectionVariableExpression::new(Some(js("x")))
        .expect("selection expression");
...
fn selection_expression_engine_path_matches_java() {
    ...
    ctx.set_variable(
        Some(js("customer")),
        Some(Arc::new(thymeleaf::expression::TemplateValue::Map(Arc::new(
            map,
        )))),
    );
    let output = engine
