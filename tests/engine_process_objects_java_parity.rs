...
fn template_manager_reprocess_and_cache_paths() {
    ...
    // 第一次 process：解析并写入缓存
    let first = engine
        .process_template("<p th:text='${name}'>a</p>", &context_with(&[("name", "one")]))
    ...
    let second = engine
        .process_template("<p th:text='${name}'>a</p>", &context_with(&[("name", "two")]))
    ...
    // 多模板交替：不同模板名互不干扰
    let other = engine
        .process_template("<span th:text='${name}'>x</span>", &context_with(&[("name", "three")]))
        .expect("other template")
        .to_string_lossy();
    assert_eq!("<span>three</span>", other);

    let third = engine
        .process_template("<p th:text='${name}'>a</p>", &context_with(&[("name", "four")]))
...
fn output_template_handler_writes_text_events() {
    ...
    let writer = CapturedWriter { buffer: buffer.clone() };
    let mut handler = OutputTemplateHandler::new(Box::new(writer));

    let text: Arc<dyn IText> = Arc::new(Text::new(Some(Arc::new(JavaString::from_rust_str("hello ")))));
    let next: Arc<dyn IText> = Arc::new(Text::new(Some(Arc::new(JavaString::from_rust_str("world")))));
...
const THROTTLED_TEMPLATES: &[(&str, &[(&str, &str)])] = &[
    ...
    ("<ul><li th:each='i : ${items}' th:text='${i}'>x</li></ul>", &[("items", "a|b|c")]),
...
fn throttled_bytes_matches_chars_encoding() {
    ...
    let written_chars = char_processor
        .process_all_writer(Box::new(CapturedWriter { buffer: buffer.clone() }))
        .expect("chars");
    ...
    let written_bytes = byte_processor
        .process_output_stream(i32::MAX, Box::new(ByteSink { buffer: bytes.clone() }), &charset)
        .expect("bytes");
    assert_eq!(
        text.as_bytes(),
        bytes.lock().unwrap_or_else(|error| error.into_inner()).as_slice()
    );
    assert_eq!(text.len() as i32, written_bytes, "UTF-8 单字节字符与 UTF-16 单元数一致");
}
...
fn throttled_processor_observers() {
    ...
    assert_eq!(
        0,
        processor
            .process_all_writer(Box::new(CapturedWriter { buffer: Arc::new(Mutex::new(Vec::new())) }))
