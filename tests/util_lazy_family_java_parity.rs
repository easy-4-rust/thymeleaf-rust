...
use thymeleaf::{TemplateResolutionAttributes, TemplateResolutionAttributeValue};
use thymeleaf::expression::TemplateValue;

use thymeleaf::processor::IProcessor;
use thymeleaf::templateresolver::StringTemplateResolver;
use thymeleaf::util::{
    FastStringWriter, IWritableCharSequence, JavaCharSequence, JavaString, LazyEscapingCharSequence,
...
fn lazy_escaping_char_sequence_html_text_xml_raw_match_java() {
    ...
    for mode in [TemplateMode::HTML, TemplateMode::TEXT] {
        ...
        assert_eq!(
            sequence.java_to_string().expect("to string").to_string_lossy(),
            "a&lt;b&gt;&amp;c&quot;d&#39;e"
        );
    }
...
fn lazy_escaping_char_sequence_null_arguments_match_java() {
    ...
    assert_eq!(error.to_string(), "Template Mode is null, which is forbidden");
}
...
fn resource_loader_utils_class_loading_matches_java() {
    ...
    assert!(ResourceLoaderUtils::is_class_present("com.example.RegisteredClass"));
    assert_eq!(
        ResourceLoaderUtils::load_class("com.example.RegisteredClass")
    ...
    assert!(!ResourceLoaderUtils::is_class_present("com.example.AbsentClass"));
    assert!(
        ResourceLoaderUtils::find_class("com.example.AbsentClass").is_none()
    );
