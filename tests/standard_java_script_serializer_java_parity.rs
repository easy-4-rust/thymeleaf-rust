...
impl JavaWriter for CapturedWriter {
    fn write_utf16(&mut self, characters: &[u16]) -> io::Result<()> {
        self.buffer.lock().unwrap_or_else(|error| error.into_inner()).extend_from_slice(characters);
        Ok(())
    }
}
...
fn serialize_value(use_jackson: bool, value: Option<&TemplateValue>) -> String {
    ...
    let mut writer = CapturedWriter { buffer: buffer.clone() };
    serializer
...
impl TemplateObject for SomeObjectB {
    ...
    fn java_serializable_properties(
        ...
        Some(vec![
            ...
            (js("three"), Some(Arc::new(TemplateValue::Number(JavaNumber::Float(1231.12))))),
...
fn print_test_enum_default_js01() {
    assert_eq!("\"FIRST\"", serialize_value(false, Some(&TemplateValue::string(js("FIRST")))));
}

/// testPrintTestEnumJacksonJS01
#[test]
fn print_test_enum_jackson_js01() {
    assert_eq!("\"FIRST\"", serialize_value(true, Some(&TemplateValue::string(js("FIRST")))));
}
...
fn script_inline_date_string_variables() {
    ...
    assert_eq!(
        "   \"something\";",
        script_inline("   /*[[${a}]]*/ 'prototype';", &[("a", string_value("something"))])
    );
}
...
fn script_inline_object_variables() {
    assert_eq!(
        ...
        script_inline(
            "   /*[[${obj01}]]*/ 'whatever';",
            &[("obj01", Arc::new(TemplateValue::Object(Arc::new(SomeObjectA))))],
        )
    );
    assert_eq!(
        ...
        script_inline(
            "   /*[[${obj02}]]*/ 'whatever';",
            &[("obj02", Arc::new(TemplateValue::Object(Arc::new(SomeObjectB))))],
        )
    );
}
