...
fn numbers_methods_match_java() {
    ...
    assert_eq!(numbers.sequence(1, 3, None).expect("sequence"), vec![1, 2, 3]);
    assert_eq!(
...
fn uris_methods_match_java() {
    ...
    assert_eq!(
        uris
            .escape_path_segment(Some(&js("a/b c")))
    ...
    assert_eq!(
        uris
            .unescape_path(Some(&js("/a%20b/c%3Fd")))
    ...
    assert_eq!(
        uris
            .escape_query_param(Some(&js("a b&c")))
            .expect("escape query")
            .expect("value")
            .to_string_lossy(),
        "a%20b%26c"
