...
fn temporal_creation_utils_matches_java() {
    ...
    let created = creation.create_date("2015-01-01", None).expect("create date");
    assert_eq!(created.kind(), JavaTemporalKind::LocalDate);
...
fn temporal_formatting_utils_matches_java() {
    ...
    assert_eq!(
        text(formatting.format(Some(&day), None, None, None).expect("format")),
        "January 1, 2015"
    );
    // GERMANY 默认日期格式
    assert_eq!(
    ...
    assert_eq!(
        text(
            formatting
                .format(Some(&day), Some("EEEE, d MMMM, yyyy"), Some(&germany()), None)
                .expect("format")
        ),
    ...
    assert_eq!(
        formatting.day_of_week(Some(&year_end)).expect("day of week"),
        Some(4),
...
fn temporal_array_utils_matches_java() {
    ...
    let formatted = array_utils
        ...
        .expect("array format");
    assert_eq!(
        formatted.iter().map(|v| v.as_ref().unwrap().to_string_lossy()).collect::<Vec<_>>(),
    ...
    assert_eq!(
        formatted.iter().map(|v| v.as_ref().unwrap().to_string_lossy()).collect::<Vec<_>>(),
        ["Donnerstag, 1 Januar, 2015", "Donnerstag, 31 Dezember, 2015"]
    );
    ...
    assert_eq!(
        array_utils.array_day_of_week(&array).expect("array day of week"),
        [Some(4), Some(4)],
