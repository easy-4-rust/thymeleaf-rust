    ...
    for text_len in 0..base.len() {
        ...
        for text_x in 0..=text_len {
            for text_y in 0..=text_len - text_x {
                ...
                assert_eq!(
                    text.len() as i32,
                    aggregate.java_length().expect("length")
                );
                ...
                for sub_x in 0..=text_len {
                    for sub_y in 0..=text_len - sub_x {
                        let expected = JavaString::from_utf16(
                            text.as_utf16()[sub_x..sub_x + sub_y].to_vec(),
                        );
                        let actual = aggregate
                            .sub_sequence(sub_x as i32, (sub_x + sub_y) as i32)
                            .expect("subSequence");
                        assert_eq!(expected, actual, "subSequence({sub_x},{}) 不匹配", sub_x + sub_y);
                    }
                }
...
fn expression_utils_member_forbidden_matches_java() {
    ...
    assert!(member_forbidden("javax.servlet.ServletContext", "someMethod"));
    assert!(member_forbidden("jakarta.servlet.ServletContext", "someMethod"));
}
