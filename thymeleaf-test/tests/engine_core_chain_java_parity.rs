...
use thymeleaf::engine::{
    ...
    ISSEThrottledTemplateWriterControl, ITemplateHandler, IThrottledTemplateWriterControl,
    ModelBuilderTemplateHandler, TemplateData, TemplateEnd, TemplateHandlerHandle, TemplateStart,
};
...
fn data_driven_template_iterator_semantics_match_java() {
    ...
    assert!(matches!(
        error,
        DataDrivenTemplateIteratorError::RemoveUnsupported
    ));
    assert_eq!(
        error.to_string(),
...
impl ISSEThrottledTemplateWriterControl for RecordingSseControl {
    fn start_event(&mut self, id: Option<&[u16]>, event: Option<&[u16]>) {
        let id = id.map(String::from_utf16_lossy).unwrap_or_default();
        let event = event.map(String::from_utf16_lossy).unwrap_or_default();
        self.events
            .lock()
            .expect("events lock")
...
impl ITemplateHandler for RecordingHandler {
    ...
    fn handle_processing_instruction(
        ...
        self.log
            .borrow_mut()
            .push("processing_instruction".to_owned());
        Ok(())
    }
}
...
fn model_builder_template_handler_collects_events_matches_java() {
    ...
    let mut builder =
        ModelBuilderTemplateHandler::new(configuration.clone(), Arc::clone(&template_data));
