...
fn load_cases() -> &'static Vec<TestCase> {
    ...
    CASES.get_or_init(|| {
        let fixture: Value = serde_json::from_str(include_str!(
            "../../thymeleaf/tests/fixtures/text_parser_test_cases.json"
        ))
...
fn check_case(case: &TestCase) {
    ...
    for process_comments in variants {
        ...
        for buffer_size in 1..=MAX_BUFFER_SIZE {
            ...
            let parse_result =
                parser.parse_reader(Some(Box::new(reader)), Some(Box::new(handler.clone())));
            assert_trace(
                &case.input,
                buffer_size,
                *process_comments,
    ...
    match parse_result {
        ...
        Err(error) => panic!(
            ...
            message(format!(
                ...
                error
                    .get_message()
                    .map_or_else(|| "null".to_owned(), |m| m.to_string_lossy())
            ))
        ),
    }
}
