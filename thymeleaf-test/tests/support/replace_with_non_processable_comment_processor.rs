...
/// 用固定 `<replaced th:text="one"/>` 模型替换 Comment 事件，替换模型不再处理。
///
/// 对应 Java:
/// `org.thymeleaf.templateengine.processors.dialects.replacewithnonprocessable.ReplaceWithNonProcessableCommentProcessor`。
pub struct ReplaceWithNonProcessableCommentProcessor {
    processor: AbstractCommentProcessor<ProcessCallback>,
}

impl ReplaceWithNonProcessableCommentProcessor {
...
impl Default for ReplaceWithNonProcessableCommentProcessor {
    fn default() -> Self {
        Self::new()
    }
}

impl IProcessor for ReplaceWithNonProcessableCommentProcessor {
...
impl ICommentProcessor for ReplaceWithNonProcessableCommentProcessor {
    fn process(
        &self,
