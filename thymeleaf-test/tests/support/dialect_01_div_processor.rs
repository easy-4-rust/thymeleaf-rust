...
/// 在每个 HTML `div` 后插入固定文本的测试 Processor。
///
/// 对应 Java:
/// `org.thymeleaf.templateengine.aggregation.dialect.Dialect01DivProcessor`。
pub struct Dialect01DivProcessor {
    processor: AbstractElementTagProcessor<ProcessCallback>,
}

impl Dialect01DivProcessor {
    /// 使用实际生效的可空方言前缀创建 Processor。
...
impl IProcessor for Dialect01DivProcessor {
...
impl IElementProcessor for Dialect01DivProcessor {
...
impl IElementTagProcessor for Dialect01DivProcessor {
    fn process(
        &self,
