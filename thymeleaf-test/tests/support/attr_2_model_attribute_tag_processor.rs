...
/// 在较早模型阶段把局部变量复制为标签属性 `var2`。
///
/// 对应 Java:
/// `org.thymeleaf.templateengine.context.dialect.Attr2ModelAttributeTagProcessor`。
pub struct Attr2ModelAttributeTagProcessor {
    processor: AbstractAttributeModelProcessor<ProcessCallback>,
}

impl Attr2ModelAttributeTagProcessor {
    /// 创建 precedence 50 的 `ctxvar:attr2model` Processor。
    pub fn new(dialect_prefix: Option<&str>) -> Self {
        Self {
            ...
            .expect("the fixed context attr2 model configuration is valid"),
        }
    }
}

impl IProcessor for Attr2ModelAttributeTagProcessor {
...
impl IElementProcessor for Attr2ModelAttributeTagProcessor {
...
impl IElementModelProcessor for Attr2ModelAttributeTagProcessor {
    fn process(
        &self,
