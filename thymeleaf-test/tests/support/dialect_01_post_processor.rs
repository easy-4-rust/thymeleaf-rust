...
pub struct Dialect01PostProcessor {
...
impl Dialect01PostProcessor {
    /// 创建计数器均为零、尚未接入处理链的后处理器。
    pub fn new() -> Self {
        Self {
    ...
    fn processing_error(
        ...
        Box::new(TemplateProcessingException::with_cause(
            Some("Could not create post-processed template event".to_owned()),
            error,
        ))
    }
...
impl Default for Dialect01PostProcessor {
    fn default() -> Self {
        Self::new()
    }
}

impl ITemplateHandler for Dialect01PostProcessor {
    ...
    fn handle_xml_declaration(
        ...
        let context = self
            .handler
            .get_context()
            .ok_or_else(|| Self::handler_error("PostProcessor context has not been set"))?;
        let encoding = Self::suffixed(
            xml_declaration.get_encoding(),
            "post",
    ...
    fn handle_doc_type(
        ...
        let context = self
            ...
            .ok_or_else(|| Self::handler_error("PostProcessor context has not been set"))?;
        let internal_subset =
            Self::suffixed(doc_type.get_internal_subset(), "post", self.doc_types);
    ...
    fn handle_cdata_section(
        ...
        let transformed = context
            .get_model_factory()
            .create_cdata_section(Self::suffixed(
                content.as_ref(),
                "post",
                self.cdata_sections,
            ))
    ...
    fn handle_comment(
        ...
        let transformed = context
            .get_model_factory()
            .create_comment(Self::suffixed(content.as_ref(), "post", self.comments))
            .map_err(Self::processing_error)?;
