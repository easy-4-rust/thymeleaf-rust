...
/// 把较晚 precedence 看到的完整模型 HTML 转义快照写入 `aggafter` 属性。
///
/// 对应 Java:
/// `org.thymeleaf.templateengine.elementprocessors.dialect.MarkupPrintAfterElementModelProcessor`。
pub struct MarkupPrintAfterElementModelProcessor {
    processor: AbstractAttributeModelProcessor<ProcessCallback>,
}

impl MarkupPrintAfterElementModelProcessor {
    /// 创建 `markup:printafter` Processor。
    pub fn new(dialect_prefix: Option<&str>) -> Self {
        Self {
            processor: AbstractAttributeModelProcessor::new(
                ...
                None,
                false,
                Some(Utf16String::from_rust_str("printafter")),
                true,
                1500,
                true,
                "org.thymeleaf.templateengine.elementprocessors.dialect.MarkupPrintAfterElementModelProcessor",
            ...
            .expect("the fixed print-after processor configuration is valid"),
        }
    }
}

impl IProcessor for MarkupPrintAfterElementModelProcessor {
...
impl IElementProcessor for MarkupPrintAfterElementModelProcessor {
...
impl IElementModelProcessor for MarkupPrintAfterElementModelProcessor {
    fn process(
        &self,
...
fn print_model(
    ...
    let tag = context
        ...
        .set_attribute(
            tag,
            Utf16String::from_rust_str("aggafter"),
            Some(markup),
            None,
        )
