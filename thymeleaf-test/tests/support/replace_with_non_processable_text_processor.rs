...
/// 用固定 `<replaced th:text="one"/>` 模型替换 Text 事件，替换模型不再处理。
///
/// 对应 Java:
/// `org.thymeleaf.templateengine.processors.dialects.replacewithnonprocessable.ReplaceWithNonProcessableTextProcessor`。
pub struct ReplaceWithNonProcessableTextProcessor {
    processor: AbstractTextProcessor<ProcessCallback>,
}

impl ReplaceWithNonProcessableTextProcessor {
...
impl Default for ReplaceWithNonProcessableTextProcessor {
    fn default() -> Self {
        Self::new()
    }
}

impl IProcessor for ReplaceWithNonProcessableTextProcessor {
...
impl ITextProcessor for ReplaceWithNonProcessableTextProcessor {
    fn process(
        &self,
