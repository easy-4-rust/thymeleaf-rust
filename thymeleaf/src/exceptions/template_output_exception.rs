...
impl TemplateOutputException {
    ...
    /// 返回完整异常消息。
    #[must_use]
    /// 对应 Java 语义：Java 接口/超类方法 `getMessage()` 的 Rust 移植（`TemplateOutputException` 继承路径）。
    ...
    /// 返回模板名称。
    #[must_use]
    /// 对应 Java 语义：Java 接口/超类方法 `getTemplateName()` 的 Rust 移植（`TemplateOutputException` 继承路径）。
    ...
    /// 判断是否存在模板名称。
    #[must_use]
    /// 对应 Java 语义：Java 接口/超类方法 `hasTemplateName()` 的 Rust 移植（`TemplateOutputException` 继承路径）。
    ...
    /// 返回行号。
    #[must_use]
    /// 对应 Java 语义：Java 接口/超类方法 `getLine()` 的 Rust 移植（`TemplateOutputException` 继承路径）。
    ...
    /// 返回列号。
    #[must_use]
    /// 对应 Java 语义：Java 接口/超类方法 `getCol()` 的 Rust 移植（`TemplateOutputException` 继承路径）。
    ...
    /// 判断行列是否同时存在。
    #[must_use]
    /// 对应 Java 语义：Java 接口/超类方法 `hasLineAndCol()` 的 Rust 移植（`TemplateOutputException` 继承路径）。
    pub fn has_line_and_col(&self) -> bool {
        self.processing.has_line_and_col()
    }

    /// 修改模板名称。
    /// 对应 Java 语义：Java 接口/超类方法 `setTemplateName()` 的 Rust 移植（`TemplateOutputException` 继承路径）。
    pub fn set_template_name(&mut self, template_name: Option<String>) {
        self.processing.set_template_name(template_name);
    }

    /// 修改行列；负数位置转换为缺失值。
    /// 对应 Java 语义：Java 接口/超类方法 `setLineAndCol()` 的 Rust 移植（`TemplateOutputException` 继承路径）。
