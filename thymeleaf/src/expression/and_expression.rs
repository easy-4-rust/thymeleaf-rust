...
/// Standard Expression 短路逻辑 AND。
///
/// 对应 Java: `org.thymeleaf.standard.expression.AndExpression`。
...
impl AndExpression {
    /// 创建 AND 表达式。
    /// 对应 Java 语义：`AndExpression` 的 `new` 行为（Rust 侧辅助/私有路径）。
    pub fn new(
    ...
    /// 返回左操作数。
    /// 对应 Java 语义：Java 接口/超类方法 `getLeft()` 的 Rust 移植（`AndExpression` 继承路径）。
    pub fn get_left(&self) -> &dyn IStandardExpression {
        self.operation.get_left()
    }
    /// 返回右操作数。
    /// 对应 Java 语义：Java 接口/超类方法 `getRight()` 的 Rust 移植（`AndExpression` 继承路径）。
    ...
    /// 对应 Java 语义：`AndExpression` 的 `is_operand_allowed` 行为（Rust 侧辅助/私有路径）。
...
impl IStandardExpression for AndExpression {
    fn get_string_representation(&self) -> StandardExpressionResult<Utf16String> {
        self.operation
            .get_string_representation(Some(&Utf16String::from_rust_str("and")))
    }
    fn execute_with_context(
        ...
        if !evaluate_as_boolean(left.as_ref())? {
            return Ok(Some(Arc::new(TemplateValue::Boolean(false))));
        }
        let right = self
            .operation
            .get_right()
...
impl ComplexExpression for AndExpression {}
