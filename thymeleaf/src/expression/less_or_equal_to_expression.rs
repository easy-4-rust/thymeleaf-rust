...
impl LessOrEqualToExpression {
    /// 创建小于或等于表达式。
    /// 对应 Java 语义：`LessOrEqualToExpression` 的 `new` 行为（Rust 侧辅助/私有路径）。
    pub fn new(
    ...
    /// 返回左操作数。
    /// 对应 Java 语义：Java 接口/超类方法 `getLeft()` 的 Rust 移植（`LessOrEqualToExpression` 继承路径）。
    pub fn get_left(&self) -> &dyn IStandardExpression {
        self.operation.get_left()
    }

    /// 返回右操作数。
    /// 对应 Java 语义：Java 接口/超类方法 `getRight()` 的 Rust 移植（`LessOrEqualToExpression` 继承路径）。
