...
use super::{
    ...
    greater_than_expression::operation_error,
};

/// Standard Expression 大于或等于表达式。
///
/// 对应 Java: `org.thymeleaf.standard.expression.GreaterOrEqualToExpression`。
pub struct GreaterOrEqualToExpression {
    operation: BinaryOperationExpression,
}

impl GreaterOrEqualToExpression {
    /// 创建大于或等于表达式。
    /// 对应 Java 语义：`GreaterOrEqualToExpression` 的 `new` 行为（Rust 侧辅助/私有路径）。
    pub fn new(
    ...
    /// 返回左操作数。
    /// 对应 Java 语义：Java 接口/超类方法 `getLeft()` 的 Rust 移植（`GreaterOrEqualToExpression` 继承路径）。
    pub fn get_left(&self) -> &dyn IStandardExpression {
        self.operation.get_left()
    }

    /// 返回右操作数。
    /// 对应 Java 语义：Java 接口/超类方法 `getRight()` 的 Rust 移植（`GreaterOrEqualToExpression` 继承路径）。
    pub fn get_right(&self) -> &dyn IStandardExpression {
        self.operation.get_right()
    }
}

impl IStandardExpression for GreaterOrEqualToExpression {
    ...
    fn execute_with_context(
        ...
        let (Some(left), Some(right)) = (left.as_ref(), right.as_ref()) else {
            return Err(operation_error(
                ...
                left.as_deref().unwrap_or(&TemplateValue::Null),
                right.as_deref().unwrap_or(&TemplateValue::Null),
            ));
        };
...
impl ComplexExpression for GreaterOrEqualToExpression {}
impl super::GreaterLesserExpression for GreaterOrEqualToExpression {}
