use super::{
    AbstractStandardAttributeModifierTagProcessor, delegate_standard_element_tag_processor,
};
use crate::TemplateMode;
use crate::util::Utf16String;
/// HTML `th:xmlspace` 到 `xml:space` 的属性修改 Processor。对应 Java: `org.thymeleaf.standard.processor.StandardXmlSpaceTagProcessor`。
pub struct StandardXmlSpaceTagProcessor {
    processor: AbstractStandardAttributeModifierTagProcessor,
}
impl StandardXmlSpaceTagProcessor {
    /// Java precedence。
    pub const PRECEDENCE: i32 = 1000;
    /// 匹配属性名。
    pub const ATTR_NAME: &'static str = "xmlspace";
    /// 目标属性名。
    pub const TARGET_ATTR_NAME: &'static str = "xml:space";
    /// 创建 Processor。
    /// 对应 Java 语义：`StandardXmlSpaceTagProcessor` 的 `new` 行为（Rust 侧辅助/私有路径）。
    pub fn new(
        dialect_prefix: Option<Utf16String>,
    ) -> Result<Self, crate::exceptions::TemplateProcessingException> {
        Ok(Self {
            processor: AbstractStandardAttributeModifierTagProcessor::with_target(
                TemplateMode::HTML,
                dialect_prefix,
                Utf16String::from_rust_str(Self::ATTR_NAME),
                Utf16String::from_rust_str(Self::TARGET_ATTR_NAME),
                Self::PRECEDENCE,
                true,
                false,
                "org.thymeleaf.standard.processor.StandardXmlSpaceTagProcessor",
            )?,
        })
    }
}
delegate_standard_element_tag_processor!(StandardXmlSpaceTagProcessor, processor);
