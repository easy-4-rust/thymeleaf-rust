...
use crate::model::IStandaloneElementTag;
...
/// 分阶段输出独立标签并在完成时降低上下文层级。
///
/// 对应 Java: `org.thymeleaf.engine.StandaloneElementTagModelProcessable`。
pub(crate) struct StandaloneElementTagModelProcessable {
...
impl StandaloneElementTagModelProcessable {
    ...
    pub(crate) fn new(
        ...
        vars: ProcessorExecutionVars,
        context: Option<Arc<dyn IEngineContext>>,
        _model_controller: Rc<RefCell<TemplateModelController>>,
        flow_controller: Arc<Mutex<TemplateFlowController>>,
        processor_template_handler: TemplateHandlerHandle,
        next_template_handler: TemplateHandlerHandle,
    ) -> Self {
        Self {
            standalone_element_tag,
            vars,
            context,
            flow_controller,
...
impl IEngineProcessable for StandaloneElementTagModelProcessable {
        ...
        if !self.delegation_processed {
            if !self.vars.discard_event {
                self.next_template_handler
                    .borrow_mut()
                    .handle_standalone_element(Arc::clone(&self.standalone_element_tag))?;
            }
