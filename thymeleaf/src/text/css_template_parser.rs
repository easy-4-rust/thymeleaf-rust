...
/// CSS 模式高层模板 Parser。
///
/// 对应 Java: `org.thymeleaf.templateparser.text.CSSTemplateParser`。
pub struct CSSTemplateParser {
    delegate: AbstractTextTemplateParser,
}

impl CSSTemplateParser {
    /// 创建启用 CSS 注释与字面量扫描的 Parser。
    /// 对应 Java 语义：`CSSTemplateParser` 的 `new` 行为（Rust 侧辅助/私有路径）。
    #[must_use]
...
impl ITemplateParser for CSSTemplateParser {
    fn parse_standalone(
        &self,
