...
| Public Rust API | Verified slices | Foundation/configuration APIs, cache families, `StandardCache`, `TemplateResolution`, template-resource SPI/string/file resources, `NumberPointType`, `IdentityCounter`, `Validate`, `MapUtils`, `ObjectUtils`, `PatternUtils`, `PatternSpec`, `VersionUtils`, and `ContentTypeUtils`; URL and JVM soft-reference runtime edges remain pending |
...
| Tests and CI | Slice gates passing | 131 unit tests, sixteen Java/Rust Golden tests with 1,986 records, 100% line/function/region coverage |
