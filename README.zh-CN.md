...
| Rust 公共 API | 已验证切片 | Foundation/配置 API、缓存对象族、`StandardCache`、`TemplateResolution`、模板资源 SPI/字符串资源/文件资源、`NumberPointType`、`IdentityCounter`、`Validate`、`MapUtils`、`ObjectUtils`、`SetUtils`/`Sets`、`PatternUtils`、`PatternSpec`、`VersionUtils` 与 `ContentTypeUtils`；URL 与 JVM 软引用运行时边界仍待补齐 |
...
| 测试与 CI | 切片门禁通过 | 138 个单元测试、17 个共 2,033 条记录的 Java/Rust Golden 测试、行/函数/区域覆盖率均为 100% |
