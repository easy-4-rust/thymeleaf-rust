...
impl HostWebRequest {
    /// 从 Axum/http 请求创建只读快照。对应 Java:
    /// `JakartaServletWebRequest#JakartaServletWebRequest`。
    ///
    /// # 参数
    /// - `request`：宿主 HTTP 请求（axum 复用 `http::Request`）。
    /// - `application_path`：Servlet context path 的中立等价文本。
    ///
    /// # 返回
    /// 保留 URI、Header、参数和 Cookie 多值顺序的请求快照。
    #[must_use]
    pub fn from_request<B>(request: &http::Request<B>, application_path: &str) -> Self {
        let uri = request.uri();
