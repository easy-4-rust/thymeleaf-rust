//! Topcoat 宿主 Web SPI 对 Java Servlet 语义的中立契约测试。
...
use thymeleaf_topcoat::{HostWebApplication, HostWebExchange, HostWebRequest, HostWebSession};
...
#[test]
fn request_preserves_uri_header_parameter_and_cookie_semantics() {
    let request = http::Request::builder()
        ...
        .header(http::header::COOKIE, "sid=one; theme=dark")
        .header(http::header::COOKIE, "sid=two")
        .body(())
...
#[test]
fn request_uses_scheme_default_port_and_root_application_path() {
    let request = http::Request::builder()
...
fn request_rejects_null_lookup_names_like_servlet_validate() {
    let request = http::Request::builder()
        ...
        .body(())
        .expect("valid request");
    HostWebRequest::from_request(&request, "").get_cookie_values(None);
}
...
/// 但宿主为 Topcoat（http 1.x 类型族） 请求/响应（策略性替代，无 Java 1:1 对照；与 thymeleaf-hyper/axum 契约逐断言对齐；与 thymeleaf-hyper 契约逐断言对齐）。
#[test]
