//! Axum 宿主 Web SPI 对 Java Servlet 语义的中立契约测试。
...
use thymeleaf_axum::{HostWebApplication, HostWebExchange, HostWebRequest, HostWebSession};
...
fn request_preserves_uri_header_parameter_and_cookie_semantics() {
    let request = axum::http::Request::builder()
        ...
        .header(axum::http::header::COOKIE, "sid=one; theme=dark")
        .header(axum::http::header::COOKIE, "sid=two")
        .body(())
...
/// 但宿主为 Axum 请求/响应（策略性替代，无 Java 1:1 对照；与 thymeleaf-hyper 契约逐断言对齐）。
#[test]
