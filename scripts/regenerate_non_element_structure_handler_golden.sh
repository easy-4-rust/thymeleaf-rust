...
java_root="${1:?usage: regenerate_non_element_structure_handler_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/non_element_structure_handler_golden.txt}"
...
javac -encoding UTF-8 -cp "${classpath}" -d "${temporary_dir}" \
    "${project_root}/thymeleaf-test/tests/java/org/thymeleaf/engine/NonElementStructureHandlerGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}:${classpath}" \
    org.thymeleaf.engine.NonElementStructureHandlerGolden > "${output}"
