...
java_root="${1:?usage: regenerate_text_parsing_attribute_sequence_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/text_parsing_attribute_sequence_golden.txt}"
...
javac -encoding UTF-8 -d "${temporary_dir}" \
    ...
    "${project_root}/thymeleaf-test/tests/java/TextParsingAttributeSequenceGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}" org.thymeleaf.templateparser.text.TextParsingAttributeSequenceGolden > "${output}"
