...
java_root="${1:?usage: regenerate_cache_key_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/cache_key_golden.txt}"
...
javac -encoding UTF-8 -d "${temporary_dir}" \
    ...
    "${thymeleaf_sources}/cache/ExpressionCacheKey.java" \
    "${thymeleaf_sources}/cache/TemplateCacheKey.java" \
    "${project_root}/thymeleaf-test/tests/java/CacheKeyGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}" CacheKeyGolden > "${output}"
