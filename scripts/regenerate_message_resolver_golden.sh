...
java_root="${1:?usage: regenerate_message_resolver_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/message_resolver_golden.txt}"
...
javac -encoding UTF-8 -cp "${classpath}" -d "${temporary_dir}" \
    "${project_root}/thymeleaf-test/tests/java/MessageResolverGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}:${classpath}" MessageResolverGolden > "${output}"
