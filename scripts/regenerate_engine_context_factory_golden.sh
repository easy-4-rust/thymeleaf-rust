...
java_root="${1:?usage: regenerate_engine_context_factory_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/engine_context_factory_golden.txt}"
...
javac -encoding UTF-8 -cp "${java_classpath}" -d "${temporary_dir}" \
    "${project_root}/thymeleaf-test/tests/java/org/thymeleaf/engine/EngineContextFactoryGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}:${java_classpath}" \
    org.thymeleaf.engine.EngineContextFactoryGolden > "${output}"
