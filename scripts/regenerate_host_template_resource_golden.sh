...
java_root="${1:?usage: regenerate_host_template_resource_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/host_template_resource_golden.txt}"
...
javac -encoding UTF-8 -d "${temporary_dir}" \
    ...
    "${project_root}/thymeleaf-test/tests/java/stubs/org/thymeleaf/templateresolver/WebApplicationTemplateResolver.java" \
    "${thymeleaf_sources}/util/Validate.java" \
    "${thymeleaf_sources}/util/ClassLoaderUtils.java" \
    "${thymeleaf_sources}/web/IWebApplication.java" \
    "${thymeleaf_sources}/templateresource/ITemplateResource.java" \
    "${thymeleaf_sources}/templateresource/TemplateResourceUtils.java" \
    "${thymeleaf_sources}/templateresource/ClassLoaderTemplateResource.java" \
    "${thymeleaf_sources}/templateresource/WebApplicationTemplateResource.java" \
    "${project_root}/thymeleaf-test/tests/java/HostTemplateResourceGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}" HostTemplateResourceGolden > "${output}"
