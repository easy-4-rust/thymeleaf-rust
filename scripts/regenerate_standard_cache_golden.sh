...
java_root="${1:?usage: regenerate_standard_cache_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/standard_cache_golden.txt}"
...
javac -encoding UTF-8 -d "${temporary_dir}" \
    ...
    "${thymeleaf_sources}/cache/ICache.java" \
    "${thymeleaf_sources}/cache/ICacheEntryValidityChecker.java" \
    "${thymeleaf_sources}/cache/StandardCache.java" \
    "${project_root}/thymeleaf-test/tests/java/StandardCacheGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}" StandardCacheGolden > "${output}"
