...
java_root="${1:?usage: regenerate_aggregate_utils_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/aggregate_utils_golden.txt}"
...
javac -encoding UTF-8 -d "${temporary_dir}" \
    ...
    "${thymeleaf_sources}/util/AggregateUtils.java" \
    "${thymeleaf_sources}/expression/Aggregates.java" \
    "${project_root}/thymeleaf-test/tests/java/AggregateUtilsGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}" AggregateUtilsGolden > "${output}"
