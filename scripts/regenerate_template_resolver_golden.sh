...
java_root="${1:?usage: regenerate_template_resolver_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/template_resolver_golden.txt}"
...
dependencies="$(cat "${temporary_dir}/dependencies.txt")"
...
javac -encoding UTF-8 -cp "${classpath}" -d "${temporary_dir}" \
    "${project_root}/thymeleaf-test/tests/java/TemplateResolverGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}:${classpath}" TemplateResolverGolden > "${output}"
