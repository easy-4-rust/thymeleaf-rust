...
java_root="${1:?usage: regenerate_abstract_processor_dialect_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/abstract_processor_dialect_golden.txt}"
...
javac -encoding UTF-8 -d "${temporary_dir}" \
    ...
    "${thymeleaf_sources}/dialect/IProcessorDialect.java" \
    "${thymeleaf_sources}/dialect/AbstractProcessorDialect.java" \
    "${thymeleaf_sources}/templatemode/TemplateMode.java" \
    "${thymeleaf_sources}/util/Validate.java" \
    "${thymeleaf_sources}/processor/IProcessor.java" \
    "${project_root}/thymeleaf-test/tests/java/AbstractProcessorDialectGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}" \
    org.thymeleaf.dialect.AbstractProcessorDialectGolden > "${output}"
