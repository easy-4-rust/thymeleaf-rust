...
java_root="${1:?usage: regenerate_context_utilities_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/context_utilities_golden.txt}"
...
javac -encoding UTF-8 -cp "${java_classpath}" -d "${temporary_dir}" \
    "${project_root}/thymeleaf-test/tests/java/org/thymeleaf/context/ContextUtilitiesGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}:${java_classpath}" \
    org.thymeleaf.context.ContextUtilitiesGolden > "${output}"
