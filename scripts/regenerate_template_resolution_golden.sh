...
java_root="${1:?usage: regenerate_template_resolution_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/template_resolution_golden.txt}"
...
javac -encoding UTF-8 -d "${temporary_dir}" \
    ...
    "${thymeleaf_sources}/templatemode/TemplateMode.java" \
    "${thymeleaf_sources}/util/Validate.java" \
    "${thymeleaf_sources}/exceptions/"*.java \
    "${thymeleaf_sources}/cache/ICacheEntryValidity.java" \
    "${thymeleaf_sources}/cache/AlwaysValidCacheEntryValidity.java" \
    "${thymeleaf_sources}/cache/NonCacheableCacheEntryValidity.java" \
    "${thymeleaf_sources}/templateresource/ITemplateResource.java" \
    "${thymeleaf_sources}/templateresolver/TemplateResolution.java" \
    "${project_root}/thymeleaf-test/tests/java/TemplateResolutionGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}" TemplateResolutionGolden > "${output}"
