...
java_root="${1:?usage: regenerate_evaluation_utils_golden.sh /absolute/path/to/thymeleaf [output]}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
project_root="$(cd "${script_dir}/.." && pwd)"
output="${2:-${project_root}/thymeleaf/tests/fixtures/evaluation_utils_golden.txt}"
...
javac -encoding UTF-8 -Xlint:unchecked -d "${temporary_dir}" \
    ...
    "${thymeleaf_sources}/standard/expression/LiteralValue.java" \
    "${thymeleaf_sources}/util/EvaluationUtils.java" \
    "${thymeleaf_sources}/expression/Bools.java" \
    "${project_root}/thymeleaf-test/tests/java/EvaluationUtilsGolden.java"

mkdir -p "$(dirname "${output}")"
java -cp "${temporary_dir}" EvaluationUtilsGolden > "${output}"
