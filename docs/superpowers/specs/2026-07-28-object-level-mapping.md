...
| 363 | `StandardTranslationDocTypeProcessor` | class | `lib/thymeleaf/src/main/java/org/thymeleaf/standard/processor/StandardTranslationDocTypeProcessor.java` |`thymeleaf/src/processor/standard_translation_doc_type_processor.rs`| `StandardTranslationDocTypeProcessor` | — | 1:1；V3 golden 差分 24/24 MATCH（`doc_type_translation_golden.txt`：16 专有 DTD SystemID 全枚举 + 大小写/PUBLIC/未知 ID/internalSubset/XML 模式边界）+ 1 已知偏差记录（HTML 模式 internal subset 不保留，html5gum 能力边界，语料零覆盖） | BEHAVIOR_VERIFIED |
