...
两个仓库已于 2026-07-30 重新建立 CodeGraph 索引。Java 上游索引包含 1,783 个文件、
29,289 个符号节点和 75,439 条关系边；Rust 索引包含 773 个文件、16,413 个符号节点和
62,165 条关系边。动态分派图用于补审 trait/接口分派、SPI 和回调路径，但不能替代固定
源码、机器清单、Java/Rust Golden 差分和真实宿主测试。
...
[迁移测试对照表](2026-07-28-migration-test-mapping.md)：
...
`baseline/migration_test_static_inventory.json`，逐项处置见
`baseline/source_parity_inventory.json`。
