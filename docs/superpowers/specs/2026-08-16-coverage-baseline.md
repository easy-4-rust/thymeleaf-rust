...
## 5. 增量复测（--all-features 档案，CI 同口径）

| 时点 | 行覆盖 | 区域 | 函数 | 未覆盖行 |
|------|-------:|-----:|-----:|---------:|
| VALUE_ADD 前（2026-08-16） | 79.89% | 75.77% | 81.11% | 21,671 |
| **VALUE_ADD 后（2026-09-03）** | **80.20%** | **76.16%** | **81.39%** | 21,344 |

97 个 VALUE_ADD 测试带来 +0.31pp / +327 行（契约导向测试，非逐行铺盖——
符合技能"覆盖率是信号"口径）。分支级归因与后续批次排期见
`2026-09-03-coverage-gap-attribution.md`。

## 6. 变更记录

| 日期 | 变更 |
|------|------|
| 2026-09-03 | 增量复测（--all-features）+ 缺口归因文档链接 |
