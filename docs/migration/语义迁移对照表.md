...
固定 Java Golden 244 条记录全部匹配；本切片与全工作区 `cargo-llvm-cov` 的行、
函数、区域覆盖率均为 100%。`AbstractTextTemplateParser` 及三种模式入口仍按
`NOT_STARTED` 管理。
...
| Processor Dialect 基础实现 | `AbstractProcessorDialect` | name/prefix/processor precedence 组合状态 + processors 扩展点 | NOT_STARTED |
...
| Text parser 基础对象语义 | 12 | 19 | 63.16% |
| Comment Reader 对象语义 | 5 | 5 | 100% |
| 通用工具对象语义 | 17 | 37 | 45.95% |
| 已实现 Java 类型 | 99 | 560 | 17.68% |
| 已实现 Java 主对象 | 90 | 491 | 18.33% |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；Foundation、方言基础配置、缓存对象族、文件/字符串模板资源与参数、求值、标准表达式字面量/执行上下文/转换服务/NO-OP 值、聚合、Array/List/Set、`#bools`/`#aggregates`/`#arrays`/`#lists`/`#sets`/`#maps`/`#objects` 表达式 facade、对象、日志、模式、版本及完整 `TextUtils` 工具切片语义已验证，其余语义保持 `NOT_STARTED`
