...
> **当前状态**：迁移实施中；Foundation 垂直切片已完成 Java/Rust Golden 差分
...
| `TemplateSpec` | template、selectors、mode、resolution attributes | 同名对象 | 构造校验、Hash/Eq、显示测试 | NOT_STARTED |
| `EngineConfiguration` | 聚合 Resolver、Dialect、Message、Link、Cache、ContextFactory | 同名不可变配置 | 配置快照与排序测试 | NOT_STARTED |
| `DialectConfiguration` | prefix 与 Dialect 配对 | 同名对象 | prefix 覆盖、冲突测试 | NOT_STARTED |
...
| Resource SPI | `ITemplateResource` | description/base/reader/exists/relative | 相对资源与重复读取 | NOT_STARTED |
| Resolution | `TemplateResolution` | resource、verified、mode、decoupled、validity | 字段覆盖组合 | NOT_STARTED |
| Template Cache | `ICache<TemplateCacheKey, TemplateModel>` | 可选、容量、有效性、统计 | hit/miss/evict/clear | NOT_STARTED |
| Expression Cache | `ExpressionCacheKey` | 表达式类型 + 输入 | 类型隔离、预处理后键 | NOT_STARTED |
| Validity | Always/TTL/Non-cacheable | 时间/永久/禁用 | 边界时钟测试 | NOT_STARTED |
...
|:---|:---|:---|:---:|
| 六个枚举值与显示名称 | 同名 enum variants 与 `Display` | 84 条 Foundation Java Golden | `BEHAVIOR_VERIFIED` |
...
| 功能 | 上游对象 | Rust 目标 | 状态 |
|:---|:---|:---|:---:|
| Dialect 基础 | `IDialect` / `AbstractDialect` | name | NOT_STARTED |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-28
**文档状态**：迁移实施中；Foundation 语义已验证，其余语义保持 `NOT_STARTED`
