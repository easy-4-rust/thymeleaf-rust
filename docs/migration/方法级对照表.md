...
`char[]` 映射为可变 `&mut [u16]`，处理器修改对解析器继续可见：

| Java 签名/方法族 | Rust API | 语义处置 | 状态 |
|:---|:---|:---|:---:|
| `ITextHandler` 11 个事件方法 | 同名 snake_case trait 方法 | 文档、文本、注释、三类元素边界与属性的全部参数原序透传；checked exception 使用 `Box<TextParseException>` 原样传播 | `BEHAVIOR_VERIFIED` |
