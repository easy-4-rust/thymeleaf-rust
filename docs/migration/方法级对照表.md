...
CodeGraph 数据库导出以下基线。当前上游仓库没有 `.codegraph/codegraph.db`，因此
JSON 是固定提交的冻结历史制品，本轮只能以源码抽查和 `migration-check` 消费它，
不能声称重生成命令当前可执行。索引恢复后必须先重生成并比较 JSON：
...
排序器。两个仓库均无 `.codegraph/`，因此本切片只声明静态调用证据，并把动态分派
缺口限定为已由双侧接口对象 Golden 覆盖的基础 getter 合同，不声称已恢复完整运行时
调用图。
...
测试留给各自对象切片。两个仓库均无 `.codegraph/`，所以当前证据不能声明完整动态
调用图；本切片只闭合构造、状态和根接口委托合同。
...
两个仓库均无 `.codegraph/`，当前仅声明静态调用面和双侧 trait/interface Golden，
不声称已恢复完整动态调用图。
...
`DialectSetConfiguration`/Processor 聚合切片。两个仓库均无 `.codegraph/`，本轮
只声明固定源码子类/调用面和双侧运行时差分，不声称完整动态调用图。
