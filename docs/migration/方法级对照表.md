...
| Java 签名 | Rust API | 语义处置 | 状态 |
|:---|:---|:---|:---:|
| `evaluateAsBoolean(Object)` | `evaluate_as_boolean(&JavaEvaluationValue)` | 显式分派 null、Boolean、全部 Number、Character、String、LiteralValue 和其他对象；空集合/数组仍为 true | `BEHAVIOR_VERIFIED` |
| `evaluateAsNumber(Object)` | `evaluate_as_number(&JavaEvaluationValue)` | `BigDecimal` 保留同一实例；整数精确转换；Float/Double 使用 `BigDecimal(double)` 精确二进制值；自定义 Number 返回 null | `BEHAVIOR_VERIFIED` |
| `evaluateAsList(Object)` | `evaluate_as_list(Option<JavaEvaluationTarget<T>>)` | null 为 `EmptyList`；非 null 为不可修改列表；Map 创建独立 `MapEntry` 快照；八种 primitive array 逐项装箱 | `BEHAVIOR_VERIFIED` |
| `evaluateAsArray(Object)` | `evaluate_as_array(Option<JavaEvaluationTarget<T>>)` | null 为单 null `Object[]`；Iterable/Map/标量新建 Object[]；引用数组返回同一实例；primitive array 抛 ClassCast | `BEHAVIOR_VERIFIED` |
| `MapEntry(K,V)` | `JavaMapEntry::new` / `raw` | `new` 对应不可变快照；`raw` 保存 Map 实现条目的运行时类和引用身份 | `BEHAVIOR_VERIFIED` |
...
`JavaEvaluationValue` 负责标量 `instanceof` 分派；`JavaEvaluationTarget`、
`JavaEvaluationElement`、`JavaEvaluationList` 和 `JavaEvaluationArray` 明确表达
JVM 集合/数组运行时类别；`JavaBigDecimalResult` 保留借用身份；`EvaluationError`
...
这一切片把 `EvaluationUtils` 中的临时 `Option<JavaString>` 字面量适配替换为
正式 `LiteralValue` 对象，并迁移表达式执行上下文的完整六状态单例图：

| Java 签名 | Rust API | 语义处置 | 状态 |
|:---|:---|:---|:---:|
| `LiteralValue(String)` | `LiteralValue::new(Option<JavaString>)` | 保留 UTF-16 文本及构造器允许 null 的行为；不建立文本值相等语义 | `BEHAVIOR_VERIFIED` |
| `String getValue()` | `get_value() -> Option<&JavaString>` | 返回原始可空文本借用 | `BEHAVIOR_VERIFIED` |
...
构造器/方法的可观察合同。`JavaTokenValue`、`JavaTokenStringResult` 和
...
| `String getStringRepresentation()` | `get_string_representation() -> Result<JavaTokenStringResult, TokenError>` | 对非 null 值调用动态 Java `toString()`；保留 null 返回、共享 String 身份、新建 String 与运行时异常 | `BEHAVIOR_VERIFIED` |
| `String toString()` | `to_string() -> Result<JavaTokenStringResult, TokenError>` | Java 实现只委托上一方法；Rust 不用 `Display` 吞掉 null/异常，因此以同一可失败 API 精确表达 | `BEHAVIOR_VERIFIED` |
...
| static `StandardInlineMode parse(String mode)` | `parse(mode: Option<&JavaString>) -> Result<StandardInlineMode, StandardInlineModeParseError>` | Java UTF-16 `trim()` 只判空、不改变匹配输入；`equalsIgnoreCase()` 的 BMP 特殊映射、null/空白/未知消息及孤立代理项由 `message()` 无损保存 | `BEHAVIOR_VERIFIED` |
...
`JavaCharSequence` trait 保留任意实现的动态 `length()`、`charAt(int)`、可变底层数据、
...
| `findCharInSequence`、三类 `isLinkBase*`、`filterOutJavaScriptLinks` | `JavaString`/UTF-16 内部辅助逻辑 | 按 Java code unit 搜索和大小写映射；只拒绝精确 scheme 前缀，不做宿主框架策略扩张 | `BEHAVIOR_VERIFIED` |
...
|:---|:---|:---|:---:|
| `IInliner#getName()` | `IInliner::get_name(&self) -> &JavaString` | 返回可识别名称；通过 trait object 动态分派 | `BEHAVIOR_VERIFIED` |
| `IInliner#inline(ITemplateContext,IText)` | `IInliner::inline_text(...)` | Text 路径独立分派；返回 nullable `JavaCharSequence`，Rust 错误进入统一表达式结果 | `BEHAVIOR_VERIFIED` |
...
方法/构造器。固定 Oracle 为
[`MessageResolverGolden.java`](../../tests/java/MessageResolverGolden.java)，109 条
记录由 [`message_resolver_java_parity.rs`](../../tests/message_resolver_java_parity.rs)
...
Oracle 为
[`NonElementStructureHandlerGolden.java`](../../tests/java/org/thymeleaf/engine/NonElementStructureHandlerGolden.java)，
...
Oracle 为
[`TemplateEngineExecutionGolden.java`](../../tests/java/TemplateEngineExecutionGolden.java)，
生成 38 条记录；Rust 由
[`template_engine_execution_java_parity.rs`](../../tests/template_engine_execution_java_parity.rs)
逐行消费。
...
固定 Java Oracle [`EngineContextGolden.java`](../../tests/java/org/thymeleaf/context/EngineContextGolden.java)
生成 41 条记录；[`engine_context_java_parity.rs`](../../tests/engine_context_java_parity.rs)
...
[`EngineEventUtilsGolden.java`](../../tests/java/org/thymeleaf/engine/EngineEventUtilsGolden.java)
固定为 14 条记录，并由
[`engine_event_utils_java_parity.rs`](../../tests/engine_event_utils_java_parity.rs) 消费。
...
的动态属性添加/删除场景由
[`ElementProcessorIteratorGolden.java`](../../tests/java/org/thymeleaf/engine/ElementProcessorIteratorGolden.java)
...
[`ElementTagStructureHandlerGolden.java`](../../tests/java/org/thymeleaf/engine/ElementTagStructureHandlerGolden.java)
