...
## 10. 模板资源 SPI 与字符串资源切片

本节覆盖 `ITemplateResource` 与 `StringTemplateResource` 的全部 11 个声明方法/
构造器。CodeGraph 已确认字符串资源只依赖资源 SPI、既有 `TemplateInputException`
和 null 校验，不需要提前引入 Resolver、Parser 或 Engine。
...
- Rust 单元测试：5；
- Java/Rust 差分测试：1；
- `cargo-llvm-cov`：当前 Rust 源码共 2,270 行、297 个函数、3,423 个区域，三项均为 100%。
