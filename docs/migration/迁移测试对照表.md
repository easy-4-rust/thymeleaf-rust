...
|:---|:---|:---|:---:|
| 上游 Core JUnit | JDK 21、`en_US`：1,154 / 1,154 | Rust 不复制 JUnit 函数计数，按下列共享资源和 Golden 消费 | 基线有效 |
| `.thtest` 共享资源 | Java 测试框架定义输入、期望输出/异常 | `semantic_all`：2,595 / 2,595 与同一期望一致 | PASS |
| 固定 Java Golden | 49 组、3,756 条 Java 记录 | Rust 逐记录读取并断言 | PASS |
...
| CI 原样全 workspace 源码覆盖率 | region 43.20%（40,596 / 93,966），function 37.04%（3,030 / 8,180），line 44.11%（31,239 / 70,819）；信息性指标 |
