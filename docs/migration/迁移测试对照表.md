...
- 消息、继承、Web exchange 与其余自定义 Processor harness；
...
| `.thtest` 固定行为批次 | Parsing 69 / 69；Plain 200 / 200；Standard Engine Context 996 / 996；专用 Dialect 61 / 61；Exception 500 / 500；`validated` 不同用例合计 1,757 / 1,757 |
...
| 2,608 个可执行 `.thtest` | `IN_PROGRESS`（1,757 个不同用例已绿灯结算：plain 200、标准 context 996、专用 Dialect 61、exception 500；其余 851 个用例独立结算） | 扩展消息、继承、自定义 Processor 与 Web exchange |
