...
该结论只覆盖固定的 Standard Engine Context 输出批次，证据级别为使用上游模板与期望
输出的适配镜像测试；不能外推为 2,608 个用例全部完成。以下范围仍独立结算：

- 消息、异常、继承、Web exchange 与自定义 Processor harness；
...
| `.thtest` 固定行为批次 | Parsing 69 / 69；Plain 200 / 200；Standard Engine Context 996 / 996；专用 Dialect 61 / 61；`verified` 不同用例合计 1,257 / 1,257 |
| 全工作区覆盖率（携带 `verified` 1,257 用例语料环境） | region 64.06%（59,683 / 93,170），function 56.46%（4,595 / 8,139），line 65.49%（46,008 / 70,257） |

覆盖率结果明确说明：生产语义已经批量落位并通过静态/编译门禁，但统一行为验证尚未
闭合。任何旧切片的 100% 都不得替代当前全工作区 65.49% 行覆盖率，也不得把
`IMPLEMENTED_UNVERIFIED` 批量改为 `BEHAVIOR_VERIFIED`。

| 范围 | 状态 | 下一动作 |
|:---|:---:|:---|
| 其余 Java JUnit 及参数化 case | `MISSING`（尚未逐项处置） | 以静态 inventory 为分母，按对象切片补行并解析参数源 |
| 2,608 个可执行 `.thtest` | `IN_PROGRESS`（1,257 个不同用例已绿灯结算：plain 200、标准 context 996、专用 Dialect 61；其余 1,351 个用例独立结算） | 扩展消息、异常、继承、自定义 Processor 与 Web exchange |
