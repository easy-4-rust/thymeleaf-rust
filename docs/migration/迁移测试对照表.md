...
| Rust 测试函数 | 294 | 含 SOURCE_PARITY、`.thtest` 清单、Golden 与 Engine 端到端门禁；不能与 Java 数量一一相减 |
...
处置分布为：`MAPPED=45`、`MERGED=93`、`SPLIT=274`、
...
| SOURCE_PARITY | 875 个源码入口、2,156 个运行时 case | 45 MAPPED、93 MERGED、274 SPLIT、1 N/A、462 POLICY_DIFFERENCE、0 MISSING | PASS |
...
| `cargo test --workspace --all-features` | 通过；核心 248 个单测，现有 Golden、清单、Engine smoke 与 3 个中立 Web 合同测试全部通过，15 个整合 crate 构建通过 |
...
| Rust 25 个人工复核候选 | `REVIEW_QUEUE`（不属于 SOURCE_PARITY 缺口） | 按测试价值量表继续确认断言、风险和所属台账 |
| CodeGraph 动态调用证据 | `UNAVAILABLE`（不属于 SOURCE_PARITY 缺口） | 仓库重新建立 `.codegraph/` 后补接口/回调路径审计 |
