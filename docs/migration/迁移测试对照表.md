...
| CI 原样全 workspace 覆盖率 | region 43.20%（40,596 / 93,966），function 37.04%（3,030 / 8,180），line 44.11%（31,239 / 70,819） |
| 核心 crate 累计携带全部 15 个语料 scope 的覆盖率 | region 72.02%（66,926 / 92,930），function 64.80%（5,202 / 8,028），line 73.44%（51,498 / 70,119）；该数字不含集成 crate，不能替代 workspace 门禁 |

覆盖率结果明确说明：生产语义与 `.thtest` 行为处置已经批量闭合，但源码覆盖率尚未
闭合。任何旧切片的 100% 或核心累计 73.44% 都不得替代当前全工作区 44.11% 行覆盖率，也不得把
