...
| Rust 测试函数 | 293 | 含 `.thtest` 清单、首批 parsing runner 与 Engine 端到端门禁；不能与 Java 数量一一相减 |
...
局部变量传播 4 / 4。这些范围彼此及前述范围均无重叠，因此当前已绿灯结算的不同
`.thtest` 是 **2,530 / 2,608**。新增批次在完整 1,757 基线上发现并修复了
...
同进程差分测试，不能外推为 2,608 个用例全部完成。以下范围仍独立结算：

- Web exchange 与其余自定义 Processor harness；
- 默认安全 OGNL 子集明确排除的任意 Java 反射链。

首批行为命令：

```bash
THYMELEAF_UPSTREAM=/Users/wandl/workspaces/workspace-github/thymeleaf \
...
python3 scripts/generate_thtest_inventory.py \
  --upstream /Users/wandl/workspaces/workspace-github/thymeleaf \
  --baseline 10f9dd2eb8cbd98515ce14b149d115e0287d0add \
  --output docs/migration/baseline/thtest_inventory.json
```

固定上游 Java Oracle 必须显式设置 JDK 与 Locale。2026-07-30 首轮在 JDK 26、
...
mvn -pl tests/thymeleaf-tests-core -am test -DskipITs \
  '-DargLine=-Duser.language=en -Duser.country=US'
```

使用上述固定环境执行全量上游 Core：**1,154 / 1,154 JUnit case 通过，0 failure，
0 error，0 skipped**。该结果证明 Java Oracle 基线有效；Rust 侧仍须逐项消费并比较，
不能把 Java 自测通过计作 Rust parity。
...
| 门禁 | 当前结果 |
|:---|:---|
| 固定 Java Oracle（JDK 21.0.12、`en_US`） | 1,154 / 1,154 通过 |
| `cargo fmt --all -- --check` | 通过 |
| `cargo clippy --workspace --all-targets -- -D warnings` | 通过 |
| `cargo test --workspace --all-targets` | 通过；核心 248 个单测，现有 Golden、清单与 Engine smoke 全部通过，15 个整合 crate 构建通过 |
...
| `.thtest` 固定行为批次 | `validated` 1,757 / 1,757；Directives 433 / 433；Pre/Post Processor 10 / 10；Multi-input 237 / 237；Link 35 / 35；Inlining Interaction 18 / 18；Conversion 28 / 28；Block 8 / 8；NoOp 4 / 4；不同用例合计 2,530 / 2,608 |
| 全工作区累计覆盖率（依次携带 `verified` 与 `exception` 语料且不清理 profile） | region 64.86%（60,520 / 93,312），function 57.43%（4,682 / 8,153），line 66.36%（46,701 / 70,377） |

覆盖率结果明确说明：生产语义已经批量落位并通过静态/编译门禁，但统一行为验证尚未
闭合。任何旧切片的 100% 都不得替代当前全工作区 66.36% 行覆盖率，也不得把
`IMPLEMENTED_UNVERIFIED` 批量改为 `BEHAVIOR_VERIFIED`。

| 范围 | 状态 | 下一动作 |
|:---|:---:|:---|
| 其余 Java JUnit 及参数化 case | `MISSING`（尚未逐项处置） | 以静态 inventory 为分母，按对象切片补行并解析参数源 |
| 2,608 个可执行 `.thtest` | `IN_PROGRESS`（2,530 个不同用例已绿灯结算；其余 78 个包含自定义 Processor、Web exchange 和具名安全排除） | 扩展其余自定义 Processor 与 Web exchange |
