...
和 38 条行为记录
[`template_engine_execution_golden.txt`](../../tests/fixtures/template_engine_execution_golden.txt)。
...
和 44 条行为记录
[`engine_configuration_golden.txt`](../../tests/fixtures/engine_configuration_golden.txt)。
...
和 46 条行为记录
[`engine_context_factory_golden.txt`](../../tests/fixtures/engine_context_factory_golden.txt)。
...
和 114 条行为记录
[`expression_objects_golden.txt`](../../tests/fixtures/expression_objects_golden.txt)。
...
和 71 条行为记录
[`basic_context_golden.txt`](../../tests/fixtures/basic_context_golden.txt)。
[`basic_context_java_parity.rs`](../../tests/basic_context_java_parity.rs) 逐项验证六个对象的
...
和 43 条行为记录
[`web_context_golden.txt`](../../tests/fixtures/web_context_golden.txt)。
[`web_context_java_parity.rs`](../../tests/web_context_java_parity.rs) 逐项验证三个对象的
...
[`MessageResolverGolden.java`](../../tests/java/MessageResolverGolden.java)、再生脚本
[`regenerate_message_resolver_golden.sh`](../../scripts/regenerate_message_resolver_golden.sh)
和行为记录
[`message_resolver_golden.txt`](../../tests/fixtures/message_resolver_golden.txt)。该记录覆盖
...
[`LinkBuilderGolden.java`](../../tests/java/LinkBuilderGolden.java)、再生脚本
[`regenerate_link_builder_golden.sh`](../../scripts/regenerate_link_builder_golden.sh) 和
[`link_builder_golden.txt`](../../tests/fixtures/link_builder_golden.txt)。63 条记录由
...
[`InlineGolden.java`](../../tests/java/InlineGolden.java)、再生脚本
[`regenerate_inline_golden.sh`](../../scripts/regenerate_inline_golden.sh) 和
[`inline_golden.txt`](../../tests/fixtures/inline_golden.txt)。23 条记录由
...
`NOT_APPLICABLE=1`、`POLICY_DIFFERENCE=462`。462 项来自
...
进入中立核心。274 项 `SPLIT` 中有 143 项直接绑定对象级 Rust 文件与类型标记，其余
131 项属于跨对象序列或测试夹具，绑定全量对象合同和共享端到端语义门禁。
...
cargo test --test thtest_upstream_plain_batch -- --nocapture
...
| 门禁 | 当前结果 |
|:---|:---|
| 固定 Java Oracle（JDK 21.0.12、`en_US`） | 五模块 2,156 / 2,156 通过；Core 1,154、Spring 5/6 各 499、Spring Security 5/6 各 2 |
| SOURCE_PARITY 清单 | 875 / 875 源码入口、2,156 / 2,156 运行时 case 完整处置，`MISSING=0` |
| `cargo fmt --all --check` | 通过 |
| `cargo clippy --workspace --all-targets --all-features -- -D warnings` | 通过 |
| `cargo test --workspace --all-features` | 通过；650 个单元/集成测试、2 个 doctest、现有 Golden、清单、Engine、中立 Web 以及 14 个整合 crate 的 28 个合同测试全部通过 |
| `THYMELEAF_SCOPE=semantic_all cargo test --test thtest_upstream_plain_batch` | 通过；2,608 个可执行 `.thtest` 全部成功或具名策略处置 |
...
| 当前全 workspace 源码覆盖率 | region 42.28%（41,913 / 99,137），function 36.46%（3,098 / 8,498），line 42.41%（31,488 / 74,252）；信息性指标 |
| 核心 crate 累计携带全部 15 个语料 scope 的源码覆盖率 | region 72.02%（66,926 / 92,930），function 64.80%（5,202 / 8,028），line 73.44%（51,498 / 70,119）；信息性指标 |
...
| RO-WEB-004 | 中立响应 → 14 个整合 crate | 状态码、Header、Body、错误脱敏和渲染器身份 | `integrations/*/tests/adapter_contract.rs` | 适配器重写状态或 Body、泄漏模板内部错误 |
...
| `integrations/*/tests/*.rs` | 28 | 14 个整合 crate、Hyper 宿主 SPI、错误/流式/拒绝策略 | 包含于最终统一结果 |
...
| **当前 `cargo test -- --list` 清单** | **1205** | 1203 个单元/集成测试 + 2 个 doctest | **覆盖率见本节最终结果** |

### 16.1 cargo-llvm-cov 最终结果

```text
TOTAL  101881  22992  77.43%  8737  2327  73.37%  75925  16101  78.79%
```
...
| TextParserTest 用例提取 | `tests/fixtures/text_parser_test_cases.json`（提取自 `TextParserTest.java`，提取脚本 `tools/extract_text_parser_test_cases.py`） | 与上游 Java 源码逐字一致（含转义还原与自洽校验） |
