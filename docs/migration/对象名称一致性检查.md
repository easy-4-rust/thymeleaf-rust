...
> **Rust 状态**：Cargo workspace 已建立；Foundation、方言基础配置、缓存合同、缓存有效性与缓存键垂直切片实施完成
...
| 状态 | 主对象 | 全部类型 | 说明 |
|:---|---:|---:|:---|
| `BEHAVIOR_VERIFIED` | 23 | 23 | Java Golden、Rust 测试和 100% slice coverage 通过 |
| `IMPLEMENTED_UNVERIFIED` | 0 | 0 | 当前没有无差分证据的实现 |
| `NOT_STARTED` | 468 | 537 | 后续 S1–S9 范围 |
| `JAVA_ONLY_EXEMPT` | 0 | 0 | 完全语义迁移目标不预设删除项 |
| `RUST_EXTENSION` | 9 | 9 | 六个类型化错误、属性值容器及两个集合别名，不计入 Java 分子 |
...
| 537 个 Java 类型尚未迁移 | 主体功能仍不可用 | S1–S9 |
| Parity runner 仅覆盖 Foundation、方言基础配置、缓存合同、缓存有效性与缓存键 | 其余对象没有 Java/Rust 差分证据 | S1 起，S11 完成 |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；23 / 491 个主对象名称与行为已验证
