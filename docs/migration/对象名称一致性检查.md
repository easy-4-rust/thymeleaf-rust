...
> **Rust 状态**：Cargo workspace 已建立；Foundation、`TemplateSpec` 与 `Thymeleaf` 垂直切片实施完成
...
| 状态 | 主对象 | 全部类型 | 说明 |
|:---|---:|---:|:---|
| `BEHAVIOR_VERIFIED` | 12 | 12 | Java Golden、Rust 测试和 100% slice coverage 通过 |
| `IMPLEMENTED_UNVERIFIED` | 0 | 0 | 当前没有无差分证据的实现 |
| `NOT_STARTED` | 479 | 548 | 后续 S1–S9 范围 |
| `JAVA_ONLY_EXEMPT` | 0 | 0 | 完全语义迁移目标不预设删除项 |
| `RUST_EXTENSION` | 5 | 5 | 两个类型化错误、属性值容器及两个集合别名，不计入 Java 分子 |
...
CI 应同时输出人类可读表格和稳定 JSON，便于比较提交之间的变化。
...
| 阻断项 | 影响 | 解除阶段 |
|:---|:---|:---:|
| 尚无完整 migration xtask | 当前由 CodeGraph JSON 清单和静态审计脚本共同检查 | S1 |
| S1 根包仍有八个对象尚未迁移 | `TemplateEngine` 与配置主体尚不可用 | S1 |
| 548 个 Java 类型尚未迁移 | 主体功能仍不可用 | S1–S9 |
| Parity runner 仅覆盖 Foundation、`TemplateSpec` 与 `Thymeleaf` | 其余对象没有 Java/Rust 差分证据 | S1 起，S11 完成 |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-28
**文档状态**：迁移实施中；12 / 491 个主对象名称与行为已验证
