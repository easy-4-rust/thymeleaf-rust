...
| 状态 | 主对象 | 全部类型 | 说明 |
|:---|---:|---:|:---|
| `BEHAVIOR_VERIFIED` | 28 | 28 | Java Golden、Rust 测试和 100% slice coverage 通过 |
| `IMPLEMENTED_UNVERIFIED` | 1 | 1 | `UrlTemplateResource` 已覆盖 file/HTTP/HTTPS，通用 URL handler 仍待迁移 |
| `NOT_STARTED` | 462 | 531 | 后续 S1–S9 范围 |
| `JAVA_ONLY_EXEMPT` | 0 | 0 | 完全语义迁移目标不预设删除项 |
| `RUST_EXTENSION` | 11 | 11 | 八个类型化错误、属性值容器及两个集合别名，不计入 Java 分子 |
...
| 533 个 Java 类型尚未迁移 | 主体功能仍不可用 | S1–S9 |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；28 / 491 个主对象名称与行为已验证，1 个主对象名称对齐且已实现待完整验证
