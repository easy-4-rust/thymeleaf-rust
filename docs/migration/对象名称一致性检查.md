...
> **Rust 状态**：Cargo workspace 已建立；Foundation、方言基础配置、缓存对象族、文件/字符串模板资源、上游五个枚举、求值、聚合、Bool/Array/List/Set/Map/Object 表达式、Token 字符语义、完整 TextUtils 及 text handler 基类垂直切片实施完成
...
| 状态 | 主对象 | 全部类型 | 说明 |
|:---|---:|---:|:---|
| `BEHAVIOR_VERIFIED` | 85 | 92 | Java Golden、Rust 测试和 100% slice coverage 通过；五个 comment Reader、内部 `BlockAction` 与完整 `TextUtils` 已闭环 |
| `IMPLEMENTED_UNVERIFIED` | 2 | 4 | `UrlTemplateResource` 待通用 URL handler；`StandardCache`（含两个内部对象）待 JVM SoftReference 自动回收等价性 |
| `NOT_STARTED` | 404 | 464 | 后续 S1–S9 范围 |
...
| 464 个 Java 类型尚未迁移 | 主体功能仍不可用 | S1–S9 |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；85 / 491 个主对象名称与行为已验证，2 个主对象名称对齐且已实现待完整验证
