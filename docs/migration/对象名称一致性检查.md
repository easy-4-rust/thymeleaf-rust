...
| 状态 | 主对象 | 全部类型 | 说明 |
|:---|---:|---:|:---|
| `BEHAVIOR_VERIFIED` | 86 | 93 | Java Golden、Rust 测试和 100% slice coverage 通过；五个 comment Reader、内部 `BlockAction`、完整 `TextUtils` 与 `IProcessor` 已闭环 |
| `IMPLEMENTED_UNVERIFIED` | 2 | 4 | `UrlTemplateResource` 待通用 URL handler；`StandardCache`（含两个内部对象）待 JVM SoftReference 自动回收等价性 |
| `NOT_STARTED` | 403 | 463 | 后续 S1–S9 范围 |
...
| 463 个 Java 类型尚未迁移 | 主体功能仍不可用 | S1–S9 |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；86 / 491 个主对象名称与行为已验证，2 个主对象名称对齐且已实现待完整验证
