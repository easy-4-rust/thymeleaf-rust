...
| 🔶 等价形态已实现 | 0 | 0 | 尚未开始实现 |
| ⬜ 缺失 | 491 | 560 | 当前事实 |
| 🚫 批准不迁移 | 0 | 0 | 完全语义迁移目标不预设删除项 |
| Rust 无来源新增 | 0 | 0 | 当前无 Rust 源码 |

当前检查的价值是建立无歧义目标，不是证明迁移完成。
...
| 阻断项 | 影响 | 解除阶段 |
|:---|:---|:---:|
| 无 Cargo workspace | 无法扫描 Rust 类型和运行测试 | S1 |
| 无 migration manifest/xtask | 名称检查目前依赖文档基线 | S1 |
| 无 Rust 源码 | 560 个类型全部缺失 | S1–S9 |
| 无 Parity runner | 无法证明行为一致 | S3 起，S11 完成 |
