...
> **Rust 状态**：Cargo workspace 已建立；Foundation 与 `TemplateSpec` 垂直切片实施完成
...
| 状态 | 主对象 | 全部类型 | 说明 |
|:---|---:|---:|:---|
| `BEHAVIOR_VERIFIED` | 11 | 11 | Java Golden、Rust 测试和 100% slice coverage 通过 |
| `IMPLEMENTED_UNVERIFIED` | 0 | 0 | 当前没有无差分证据的实现 |
| `NOT_STARTED` | 480 | 549 | 后续 S1–S9 范围 |
...
| S1 根包仍有九个对象尚未迁移 | `TemplateEngine` 与配置主体尚不可用 | S1 |
| 549 个 Java 类型尚未迁移 | 主体功能仍不可用 | S1–S9 |
| Parity runner 仅覆盖 Foundation 与 `TemplateSpec` | 其余对象没有 Java/Rust 差分证据 | S1 起，S11 完成 |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-28
**文档状态**：迁移实施中；11 / 491 个主对象名称与行为已验证
