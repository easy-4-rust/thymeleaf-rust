...
> **Rust 状态**：S1–S10 生产对象已按语义域批量落位；核心 `thymeleaf` 与全部框架/Vernal 整合 crate 可编译。当前进入统一账本收口与 S11 行为验证准备，不据静态存在性声明兼容完成。
...
| 内部/伴随对象 | 69 | 固定基线源码与机器清单；当前 CodeGraph 索引不可用 |
...
| 状态 | 主对象 | 全部类型 | 说明 |
|:---|---:|---:|:---|
| `BEHAVIOR_VERIFIED` | 89 | 96 | Java Golden、Rust 测试和 100% slice coverage 通过；五个 comment Reader、内部 `BlockAction`、完整 `TextUtils`、Processor 基础对象、`IProcessorDialect` 与 `AbstractProcessorDialect` 已闭环 |
...
### 1.3 实时静态门禁（2026-07-30）

`xtask migration-check` 对当前工作树重新扫描后通过：

| 维度 | 当前值 |
|:---|---:|
| 主对象 exact / approved equivalent | 472 / 19 |
| `BEHAVIOR_VERIFIED` / `IMPLEMENTED_UNVERIFIED` / `JAVA_ONLY_EXEMPT` | 89 / 390 / 12 |
| missing / type mismatch / stub / path collision | 0 / 0 / 0 / 0 |
| Rust extension/辅助文件 | 24 |
| Java 方法 declared / review required | 4,291 / 0 |

24 个 Rust extension/辅助文件包含 Java/Rust 运行时边界对象及共享签名别名，例如
`ContextVariableEntries`，以及 OGNL 集合桥接使用的 `JavaIterator`、`JavaMapEntry`
和 `JavaStream`。本轮新增的 `OgnlException`、`ClassNotFoundException` 与
`NoSuchMethodException` 对应外部 OGNL/JDK 异常身份，也不计入 Thymeleaf Java 对象
完成分子。静态门禁通过只证明对象和方法都有明确处置，不把 390 个待验证对象升级为
行为已验证。
...
| 维度 | 当前 |
|:---|---:|
| 已实现且名称直接对齐的 Java 主对象 | 472 |
| 已登记批准等价名称的 Java 主对象 | 19 |
...
| 阻断项 | 影响 | 解除阶段 |
|:---|:---|:---:|
| 390 个核心生产对象尚无统一差分证据 | 只能标记 `IMPLEMENTED_UNVERIFIED`，不能声明兼容 | S11 |
| 12 个 Servlet 宿主等价映射尚待统一整合矩阵 | 可标记 `JAVA_ONLY_EXEMPT`，但不能跳过宿主行为验证 | S11 |
| Java 测试类、资源和 fixture 尚未形成统一迁移清单 | 可能漏掉非 `.thtest` 行为合同 | S11 |
| 2,608 个可执行 `.thtest` 尚有 851 个未结算 | 无法声明模板处理兼容 | S11 |
| 全 workspace 累计 `cargo llvm-cov` 仅为 region 64.86%、function 57.43%、line 66.36% | 无法发布最终覆盖率和兼容结论 | S11 |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-30
**文档状态**：491 / 491 个主对象与 69 / 69 个内部对象已有生产落点；89 个主对象保留既有行为验证证据，其余统一进入 S11 验证
