...
> **Rust 状态**：Cargo workspace 已建立；Foundation、方言基础配置、缓存对象族、文件/字符串模板资源、上游五个枚举、求值、聚合、Bool/Array/List/Set/Map/Object 表达式及 Token 字符语义垂直切片实施完成
...
| 状态 | 主对象 | 全部类型 | 说明 |
|:---|---:|---:|:---|
| `BEHAVIOR_VERIFIED` | 72 | 76 | Java Golden、Rust 测试和 100% slice coverage 通过；新增 text parser 属性序列解析切片 |
| `IMPLEMENTED_UNVERIFIED` | 2 | 4 | `UrlTemplateResource` 待通用 URL handler；`StandardCache`（含两个内部对象）待 JVM SoftReference 自动回收等价性 |
| `NOT_STARTED` | 417 | 480 | 后续 S1–S9 范围 |
| `JAVA_ONLY_EXEMPT` | 0 | 0 | 完全语义迁移目标不预设删除项 |
| `RUST_EXTENSION` | 74 | 74 | 包括 `TextParseCause`、`TextParsingCommentError`、`TextParsingUtilError`、`TextParsingAttributeSequenceError` 等 Rust/JVM 边界适配，不计入 Java 分子 |
...
| 485 个 Java 类型尚未迁移 | 主体功能仍不可用 | S1–S9 |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；72 / 491 个主对象名称与行为已验证，2 个主对象名称对齐且已实现待完整验证
