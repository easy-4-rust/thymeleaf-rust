...
| 状态 | 主对象 | 全部类型 | 说明 |
|:---|---:|---:|:---|
| `BEHAVIOR_VERIFIED` | 36 | 37 | Java Golden、Rust 测试和 100% slice coverage 通过；`VersionSpec` 为紧耦合内部对象 |
| `IMPLEMENTED_UNVERIFIED` | 2 | 4 | `UrlTemplateResource` 待通用 URL handler；`StandardCache`（含两个内部对象）待 JVM SoftReference 自动回收等价性 |
| `NOT_STARTED` | 453 | 519 | 后续 S1–S9 范围 |
| `JAVA_ONLY_EXEMPT` | 0 | 0 | 完全语义迁移目标不预设删除项 |
| `RUST_EXTENSION` | 18 | 18 | 十三个类型化错误、属性值容器、两个集合别名、Java `Pattern` 编译结果包装及 UTF-16 版本限定符包装，不计入 Java 分子 |
