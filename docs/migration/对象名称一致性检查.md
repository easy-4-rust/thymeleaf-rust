...
| Rust extension/辅助文件 | 21 |
| Java 方法 declared / review required | 4,291 / 0 |

21 个 Rust extension/辅助文件包含 Java/Rust 运行时边界对象及共享签名别名，例如
`ContextVariableEntries`，以及 OGNL 集合桥接使用的 `JavaIterator`、`JavaMapEntry`
和 `JavaStream`；它们不计入 Java 对象完成分子。静态门禁通过只证明对象和方法都有
明确处置，不把 390 个待验证对象升级为行为已验证。
...
| 2,608 个可执行 `.thtest` 尚有 1,351 个未结算 | 无法声明模板处理兼容 | S11 |
| 全 workspace `cargo llvm-cov` 与差异登记尚未完成 | 无法发布最终覆盖率和兼容结论 | S11 |
