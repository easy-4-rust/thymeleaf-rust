...
- 187 个 Rust 单元测试和 28 个 Java/Rust 差分测试通过，共固定 2,567 条 Java 记录；
- 当前 Rust 源码 9,862 行、1,154 个函数、15,247 个区域，覆盖率均为 100%；
- 完整 Java API 清单登记 4,291 个方法/构造器和 6,936 个参数。
- `cargo xtask migration-check` 已进入 CI，当前报告 61 个已验证对象、2 个已实现待完整验证对象、0 路径冲突、0 STUB。
...
| 核心 API | S1 Foundation、方言基础配置，S2 缓存/解析/资源切片、上游五个 enum，以及 S6 参数、求值、聚合、Array/List/Set、`#bools`/`#aggregates`/`#arrays`/`#lists`/`#sets`/`#maps`/`#objects`、对象、模式、版本与 Token 字符语义已验证；URL 资源和 `StandardCache` 的 JVM 运行时边界待完整验证；Engine 尚未实现 |
| 当前源码覆盖率 | 行 / 函数 / 区域 100% |
| 兼容性声明 | 不允许 |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；61 / 491 个主对象已验证，2 个主对象已实现待完整验证
