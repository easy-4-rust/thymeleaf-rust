...
- 235 个 Rust 单元测试和 44 组固定 Java Oracle 通过，共登记 3,650 条 Java 记录；
- 当前 Rust 源码 21,480 行、1,967 个函数、29,232 个区域，覆盖率均为 100%；
- 完整 Java API 清单登记 4,291 个方法/构造器和 6,936 个参数。
- `cargo xtask migration-check` 已进入 CI，当前报告 84 个已验证对象、2 个已实现待完整验证对象、0 路径冲突、0 STUB。
...
`TextParser`（含紧耦合 `BufferPool`）、reader 包 5 / 5 个对象以及
`IInlinePreProcessorHandler` 已经沿真实 text/markup parser 调用链完成迁移。下一步继续
`AbstractTextTemplateParser`、`TextTemplateParser`、JavaScript/CSS 入口和
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；84 / 491 个主对象已验证，2 个主对象已实现待完整验证
