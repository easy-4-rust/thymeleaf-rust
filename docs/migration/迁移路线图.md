...
- 147 个 Rust 单元测试和 18 个 Java/Rust 差分测试通过，共固定 2,099 条 Java 记录；
- 当前 Rust 源码 6,919 行、808 个函数、10,857 个区域，覆盖率均为 100%；
- 完整 Java API 清单登记 4,291 个方法/构造器和 6,936 个参数。
- `cargo xtask migration-check` 已进入 CI，当前报告 41 个已验证对象、2 个已实现待完整验证对象、0 路径冲突、0 STUB。
...
- `NumberPointType`、`IdentityCounter`、`Validate`、`MapUtils`、`ObjectUtils`、
  `ListUtils`、`Lists`、`SetUtils`、`Sets`、`PatternUtils`、`PatternSpec` 与 `VersionUtils` 共 12 / 64 个 S6 主对象达到
  `BEHAVIOR_VERIFIED`；
- 固定 Java Oracle 生成 968 条 Golden，覆盖五个枚举成员及顺序、严格名称匹配、
  引用 alias、值相等但身份不同、null 身份、容量边界，以及八个参数校验重载的成功与
  异常路径，并覆盖 Map 全部查询重载、null 安全对象选择、字符串模式转换及有序模式
  集合的匹配与失败后状态、List/Set 转换与 `#lists`/`#sets` 委托，以及版本解析、限定符、构建时间戳和三层版本比较；
...
| 核心 API | S1 Foundation、方言基础配置，S2 缓存/解析/资源切片及 S6 参数、List/Set、`#lists`/`#sets`、对象、模式、版本工具已验证；URL 资源和 `StandardCache` 的 JVM 运行时边界待完整验证；Engine 尚未实现 |
| 当前源码覆盖率 | 行 / 函数 / 区域 100% |
| 兼容性声明 | 不允许 |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；41 / 491 个主对象已验证，2 个主对象已实现待完整验证；其中 S1 完成 13 / 20、S2 验证 13 / 30（已实现 15 / 30）、S6 验证 12 / 64
