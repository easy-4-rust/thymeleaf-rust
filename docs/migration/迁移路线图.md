...
- 230 个 Rust 单元测试和 42 组固定 Java Oracle 通过，共登记 3,564 条 Java 记录；
- 当前 Rust 源码 20,835 行、1,925 个函数、28,544 个区域，覆盖率均为 100%；
- 完整 Java API 清单登记 4,291 个方法/构造器和 6,936 个参数。
- `cargo xtask migration-check` 已进入 CI，当前报告 81 个已验证对象、2 个已实现待完整验证对象、0 路径冲突、0 STUB。
...
- `BlockAwareReader`、`ParserLevelCommentTextReader` 与
  `PrototypeOnlyCommentTextReader` 共 3 / 41 个 S4 主对象达到
  `BEHAVIOR_VERIFIED`，内部 `BlockAction` 与主对象同文件；
- `/*[-`…`-]*/` 全丢弃和 `/*[+`…`+]*/` 仅丢容器语义通过固定 Java
  Oracle、UTF-16、任意 Reader 切分、未闭合结构、读取/关闭失败验证；
- `ParserLevelCommentMarkupReader`、`PrototypeOnlyCommentMarkupReader` 以及
  `AbstractTextTemplateParser` 等其余 38 个 S4 主对象保持 `NOT_STARTED`。
...
| 核心 API | S1 Foundation、方言基础配置，S2 缓存/解析/资源切片、text parser 状态/定位/异常/处理器接口/默认与链式 handler/事件结构预处理/注释事件预处理/注释解析/字面量判定/通用结构扫描/属性序列解析/元素解析、上游五个 enum、`FastStringWriter`、`CharArrayWrapperSequence`，以及 S6 参数、求值、聚合、Array/List/Set、`#bools`/`#aggregates`/`#arrays`/`#lists`/`#sets`/`#maps`/`#objects`、对象、模式、版本与 Token 字符语义已验证；URL 资源和 `StandardCache` 的 JVM 运行时边界待完整验证；Engine 尚未实现 |
| 当前源码覆盖率 | 行 / 函数 / 区域 100% |
| 兼容性声明 | 不允许 |

`TextParser`（含紧耦合 `BufferPool`）以及它上游直接使用的两个文本注释 Reader
已经沿真实 text parser 调用链完成迁移。下一步先迁移同一状态机的两个 markup
Reader，再继续 `AbstractTextTemplateParser`、`TextTemplateParser`、
JavaScript/CSS 入口和模板管理器调用链；URL 资源协议 handler、
S2 Cache/Resolver 依赖链、S1 根包与 S5 方言 SPI 仍保留在后续阶段。Java API
JSON、静态布局审计与覆盖率检查已经进入正式 CI 门禁。
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；81 / 491 个主对象已验证，2 个主对象已实现待完整验证
