...
- 30 个 Rust 单元测试和 4 个 Java/Rust 差分测试通过；
- 当前 Rust 源码的行、函数、区域覆盖率均为 100%；
- 完整 Java API 清单登记 4,291 个方法/构造器和 6,936 个参数。
- `cargo xtask migration-check` 已进入 CI，当前报告 15 个已验证对象、0 路径冲突、0 STUB。
...
| 核心 API | S1 Foundation、`TemplateSpec`、`Thymeleaf`、方言基础配置已验证；Engine 尚未实现 |
| 当前源码覆盖率 | 行 / 函数 / 区域 100% |
| 兼容性声明 | 不允许 |

下一步继续 S1 根包其余七个对象，并按依赖闭合顺序扩展 S5 方言 SPI；Java API JSON、静态布局审计与覆盖率检查已经进入正式 CI 门禁。
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；15 / 491 个主对象已验证，其中 S1 完成 13 / 20
