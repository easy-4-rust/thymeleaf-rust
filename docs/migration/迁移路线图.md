...
`migration-check` 均通过；固定 Java Oracle 为 1,154 / 1,154；全 workspace
依次携带 `verified` 与 `exception` 语料、不清理 profile 累计执行的
`cargo-llvm-cov` 为 region 64.86%、function 57.43%、line 66.36%。因此当前状态仍是
...
`validated` 并集因此为 **1,757 / 1,757**。剩余 851 个仍需按消息、继承、自定义
Processor 和 Web exchange 域结算，不能外推为全部 2,608 个用例完成。Java `Class`
反射链不纳入默认安全 OGNL 子集，相关差异
必须显式登记，禁止为追求兼容数字开放任意反射。
