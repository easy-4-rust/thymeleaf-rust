...
> **目标分支**：`dev`
> **状态**：迁移实施中；Foundation 与 `TemplateSpec` 垂直切片已完成行为验证
...
- 根包 `TemplateSpec` 达到 `BEHAVIOR_VERIFIED`，其余 9 个对象保持 `NOT_STARTED`；
- Foundation Java Oracle 生成 86 条 Golden 记录；
- `TemplateSpec` Java Oracle 生成 289 条 Golden 记录；
- 23 个 Rust 单元测试和 2 个 Java/Rust 差分测试通过；
...
| 核心 API | S1 Foundation 与 `TemplateSpec` 已验证；Engine 尚未实现 |
| 当前源码覆盖率 | 行 / 函数 / 区域 100% |
| 兼容性声明 | 不允许 |

下一步继续 S1 根包其余九个对象，并将现有 Java API JSON、静态布局审计与覆盖率检查整合为正式 xtask 门禁。
