...
- 246 个 Rust 单元测试和 48 组固定 Java Oracle 通过，共登记 3,751 条 Java 记录；
- 当前 Rust 源码 23,651 行、2,105 个函数、32,081 个区域，覆盖率均为 100%；
- 完整 Java API 清单登记 4,291 个方法/构造器和 6,936 个参数。
- `cargo xtask migration-check` 已进入 CI，当前报告 88 个已验证对象、2 个已实现待完整验证对象、0 路径冲突、0 STUB。
...
- `AbstractProcessorDialect` 仍为 `NOT_STARTED`，后续以组合状态迁移，不能把
  已完成的接口误当作基础实现；
- `OutputExpressionInlinePreProcessorHandler` 仍依赖名称仓库、转义工具及标准
  Processor 常量，保持 `NOT_STARTED`，不得以局部算法替代完整对象。
...
| 核心 API | S1 Foundation、方言基础配置、Processor 基础契约（`IProcessor`/`AbstractProcessor`）与 `IProcessorDialect`，S2 缓存/解析/资源切片、text parser 状态/定位/异常/处理器接口/默认与链式 handler/事件结构预处理/注释事件预处理/注释解析/字面量判定/通用结构扫描/属性序列解析/元素解析、上游五个 enum、`FastStringWriter`、`CharArrayWrapperSequence`、完整 `TextUtils`，以及 S6 参数、求值、聚合、Array/List/Set、`#bools`/`#aggregates`/`#arrays`/`#lists`/`#sets`/`#maps`/`#objects`、对象、模式、版本与 Token 字符语义已验证；URL 资源和 `StandardCache` 的 JVM 运行时边界待完整验证；Engine 尚未实现 |
| 当前源码覆盖率 | 行 / 函数 / 区域 100% |
| 兼容性声明 | 不允许 |

`TextParser`（含紧耦合 `BufferPool`）、reader 包 5 / 5 个对象、
`IInlinePreProcessorHandler`、Processor 基础对象 `IProcessor`/`AbstractProcessor`
与 Processor 方言根契约 `IProcessorDialect` 已完成迁移。下一步优先完成
`AbstractProcessorDialect`，再继续 `AbstractTextTemplateParser`、
`TextTemplateParser`、JavaScript/CSS 入口和
模板管理器调用链，并为 markup parser 的调用链准备 attoparser 语义；URL 资源协议 handler、
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；88 / 491 个主对象已验证，2 个主对象已实现待完整验证
