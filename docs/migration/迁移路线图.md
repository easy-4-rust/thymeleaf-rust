...
- 48 个 Rust 单元测试和 6 个 Java/Rust 差分测试通过，共固定 495 条 Java 记录；
- 当前 Rust 源码 2,017 行、264 个函数、3,011 个区域，覆盖率均为 100%；
- 完整 Java API 清单登记 4,291 个方法/构造器和 6,936 个参数。
- `cargo xtask migration-check` 已进入 CI，当前报告 21 个已验证对象、0 路径冲突、0 STUB。
...
- `ICacheEntryValidity`、`AlwaysValidCacheEntryValidity`、
  ...
  `TemplateCacheKey` 共 6 / 30 个 S2 主对象达到 `BEHAVIOR_VERIFIED`；
...
| 核心 API | S1 Foundation、方言基础配置与 S2 缓存有效性/缓存键已验证；Engine 尚未实现 |
| 当前源码覆盖率 | 行 / 函数 / 区域 100% |
| 兼容性声明 | 不允许 |
...
**创建日期**：2026-07-28
**最后更新**：2026-07-29
**文档状态**：迁移实施中；21 / 491 个主对象已验证，其中 S1 完成 13 / 20、S2 完成 6 / 30
