...
const PREFIX: &[u16] = &[0x002f, 0x002a, 0x005b, 0x002b];
const SUFFIX: &[u16] = &[0x002b, 0x005d, 0x002a, 0x002f];

/// 删除文本模板中的仅原型注释容器，同时保留容器内容。
///
/// 识别 `/*[+`…`+]*/`，并且在任意底层 Reader/调用方缓冲边界下保持相同结果。
/// 对应 Java:
/// `org.thymeleaf.templateparser.reader.PrototypeOnlyCommentTextReader`。
pub struct PrototypeOnlyCommentTextReader {
    delegate: BlockAwareReader,
}

impl PrototypeOnlyCommentTextReader {
    /// 包装一个 Reader，并启用仅原型文本注释容器过滤。
    ///
    /// 参数 `reader` 为底层 UTF-16 Reader；返回可继续交给 `TextParser` 或其他
    /// Reader 包装器的对象。对应 Java: `PrototypeOnlyCommentTextReader#PrototypeOnlyCommentTextReader`。
    #[must_use]
...
impl TextParserReader for PrototypeOnlyCommentTextReader {
    fn read_range(
        &mut self,
        buffer: &mut [u16],
        offset: i32,
